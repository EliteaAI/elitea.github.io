...
description: "Execution Nodes enable your pipeline to perform actions, call external tools, execute code, and integrate with APIs. These nodes form the \"action\" layer of yo"
---
...
Every tool-executing node (Toolkit, MCP) writes a typed outcome into pipeline state beside its normal output. This removes the need to parse the message history to determine whether a tool call failed.

### New Pipeline State Keys

| Key | Shape | Meaning |
|-----|-------|---------|
| **last_tool_outcome** | One outcome object | The most recent tool node's outcome |
| **tool_outcomes** | `{ node_id: outcome }` | One entry per node that ran, merged across the run |

<Warning title="Add These State Variables Manually">
Unlike `input` and `messages`, `last_tool_outcome` and `tool_outcomes` are not added to the pipeline's `state` section automatically. Add them yourself before referencing them elsewhere in the pipeline. See **[Pipeline States](../states)** for how to manage state variables.
...
<Note>
Both keys are cleared at the start of every turn, so a previous turn's failure cannot leak into a condition. Re-entering a node (a loop) overwrites that node's entry.
</Note>
...
|-------|--------|------------|
| **status** | `success` \| `error` \| `truncated` \| `blocked` | The one field to branch on |
| **error_class** | `infrastructure` \| `input` \| `tool_internal` \| `policy` \| `null` | What kind of failure occurred |
| **retriable** | `true` \| `false` | `true` only for infrastructure failures — retrying identical input could work |
| **message** | Prose | The same human/LLM-facing text as before (capped at 4000 characters) |
| **tool_name**, **toolkit_type** | String \| `null` | Which tool failed |
| **exception_type** | Class name \| `null` | For example, `ReadTimeout`, `ValueError` — never a traceback |
| **message_truncated**, **message_original_size** | Present only when the message was capped | Detect and inspect truncated messages |

**Example: Routing on Tool Outcome**
...
nodes:
  - id: call_api
    type: toolkit
    # ...
    transition: check_outcome

  - id: check_outcome
    type: decision
    condition: "{{ last_tool_outcome.status == 'error' and last_tool_outcome.retriable }}"
    if_true: retry_call
    if_false: continue_flow
```

<Tip title="Branch on error_class, not on message text">
Use `last_tool_outcome.status` and `last_tool_outcome.error_class` to route pipeline logic. Avoid matching on `message` text, since prose wording can change between tool versions.
</Tip>
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/toolkit-node-add.gif" alt="Toolkit Node Interface" />
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/toolkit-node-interface.png" alt="Toolkit node" width="300" />
...
<Info title="Input Mapping Configuration">
...
<Warning title="Single Toolkit and Tool Selection">
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/mcp-node-add.gif" alt="MCP Node Interface" />
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/mcp-node-interface.png" alt="MCP node" width="300" />
...
<Info title="MCP Connection Required">
...
<Warning title="Single MCP and Tool Selection">
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/custom-node-add.gif" alt="MCP Node Interface" />
...
<Warning title="Advanced Users Only">
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/code-node-add.gif" alt="Code Node Interface" />
...
| **Debug** | Save the fully assembled executable code snapshot before sandbox execution | **Enabled (true):** Saves a standalone Python file to the `code-debug` artifact bucket before execution<br />**Disabled (false):** No debug artifact is created<br /><br />**Saved file includes:**<br />- Sandbox client preamble<br />- Injected `elitea_state` / `alita_state` preamble<br />- Your node code wrapped for local execution<br /><br />**Filename format:** `<node_id>__<YYYYMMDD_HHMMSS>.py` |
...
<img src="../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/code-node-interface.png" alt="Code Node Interface" />

### Debug Mode

Use Code Node debug mode when you need to inspect the exact Python source that ELITEA sent to the sandbox.

<Note title="What debug mode does">
...
- Run the file locally to reproduce the exact execution context

   ![Debug Mode Example](../../../img/how-tos/agents-pipelines/pipeline-building-blocks/nodes/code-node-debug.gif)
...
nodes:
  - id: process_data
    ...
    code:
      type: fixed
      value: |
        data = alita_state.get('raw_input', '')
...
<Info title="Debug Artifact Contents">
...
<Accordion title="Basic Example: Simple Calculation">
```yaml
nodes:
  - id: calculate_total
    type: code
...
<Accordion title="Advanced Example: Data Processing with Error Handling">
```yaml
nodes:
...
<Accordion title="API Integration Example: External HTTP Calls">
```yaml
nodes:
  - id: fetch_api_data
    type: code
...
<Accordion title="Package Installation Example: Using External Libraries">
```yaml
nodes:
  - id: data_analysis
    type: code
...
<Accordion title="Conditional Logic Example: Business Rules">
```yaml
nodes:
...
<Accordion title="Alita Client Integration Example: Accessing Artifacts">
```yaml
nodes:
  - id: list_artifacts
    type: code
...
<Info title="Output Handling: With or Without Output Variables">
...
<Tip title="State Access & Alita Client (SandboxClient) Capabilities">
**State Access:**
...
<Warning title="Structured Output Behavior">
...
<Info title="Code Execution Environment">
...
<Accordion title="Error: Deno is required for PyodideSandbox but is not installed">
...
<Accordion title="Error: Object is not serializable">
...
<Accordion title="Error: ModuleNotFoundError: No module named 'package'">
...
<Accordion title="Error: Output variables not updating">
...
<Accordion title="Issue: Debug mode is enabled but no debug artifact appears">
**Cause:** Debug artifact capture is best-effort. The node saves the file only when `debug: true` is set and the runtime has access to the artifact client. Upload failures do not stop pipeline execution.
...
<Accordion title="Issue: The downloaded debug file does not run locally">
**Cause:** The saved file contains an auth token placeholder and may rely on packages that are not installed in your local Python environment.
...
<Accordion title="Error: httpx or network requests failing">
...
**Migration Guide:** [Function Node Migration](../../../migration/v2.0.1/function-node-migration)
</Accordion>

<Accordion title="Tool Node">
...
**Migration Guide:** [Tool Node Migration](../../../migration/v2.0.1/tool-node-migration)
</Accordion>

---

<Info title="Related">
- **[Nodes Overview](./overview)** - Understand all available node types
- **[Interaction Nodes](./interaction-nodes)** - LLM and Agent nodes for AI-powered tasks
- **[Control Flow Nodes](./control-flow-nodes)** - Router, and Decision nodes
- **[States](../states)** - Manage data flow through pipeline state
- **[Connections](../nodes-connectors)** - Link nodes together
- **[YAML Configuration](../yaml)** - See complete node syntax examples
</Info>
