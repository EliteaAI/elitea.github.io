...
description: "Entity versioning (for agents and pipelines) in ELITEA allows you to create, manage, and track different iterations of your agents. This feature enables you to "
---

## Overview

<Note>
**Applies to Skills Too:** While this guide focuses on agent and pipeline versioning, **skill versioning works exactly the same way**. All patterns, naming conventions, version management operations, and best practices described here apply equally to skills.
</Note>
...
<Info title="Version Naming Rules">
...
3. **Click Save**: The new version is created and becomes the current active version. The system automatically navigates to the newly created version, updates the version dropdown to show the new version, and updates the URL to reflect the new version ID. You can immediately continue editing the new version or switch to another version.
     ...
     ![Creating a new version](../../img/how-tos/entity-management/entity-versioning/create-new-version.png)
...
The version dropdown is located in the top toolbar. Click any version to switch and view its configuration. When you switch, the system loads the selected version's complete configuration, updates the URL to include the new version ID, updates all form fields to show the selected version's data, and updates the breadcrumb navigation. Any unsaved changes in the previous version are preserved (you'll see them if you switch back).
  *  Each version displays:
     - **Version name** (e.g., "v1.0", "base")
     - **Creation metadata** in format: "Month DD, YYYY, HH:MM · by Author Name" (e.g., "Mar 15, 2024, 14:30 · by John Smith")
     - **Visual indicators**: Pin icon for default version, publish icon for published versions
  *  Includes a search field with placeholder to quickly find specific versions

Versions are sorted by creation date (newest first), with "base" always appearing at the bottom:

     ![Version Selection](../../img/menus/agents/version-select.png)
...
<Tip title="Benefits of Setting a Default Version">
...
5. Click **"Set as a default"** to confirm
     ...
     ![ Default Version](../../img/how-tos/entity-management/entity-versioning/select-default-version-1.png)
...
5. Click **"Set as a default"** to confirm
     ...
     ![ Default Version](../../img/how-tos/entity-management/entity-versioning/select-default-version-2.png)


The default version will now be marked with a pin icon (📌) in the version dropdown.

<Info title="Default Version Behavior">
* **Automatic Selection**: The default version is automatically used in all new integrations
* **Visual Indicator**: Marked with a pin icon in the version dropdown (stays in its chronological position, not pinned to top)
...
Publishing makes an agent version available in [Agent HUB](../../menus/agents-studio), where it can be discovered and used by all ELITEA users.

<Info title="Agent-Only Feature">
Publishing is available only for agents. Pipelines do not have publish functionality.
</Info>

**Key Features:**

* **Guided Three-Step Wizard**: Publish through preparation, AI-powered validation, and submission steps
* **AI-Powered Validation**: Automated validation identifies issues and improvement areas before publishing
* **Category Selection**: Agents are organized by category for easy discovery in Agent HUB
* **Publishing Terms**: Review what's included/excluded and best-practice requirements
* **Permission Required**: You must have `models.applications.publish.post` permission

**Quick Publishing Flow:**

1. Open the agent and select the version to publish
2. Click the **three-dot menu (⋮)** and select **Publish**
3. Complete the three-step wizard:
   - **Preparation**: Enter version name, select category, agree to terms
   - **Validation**: AI review checks configuration and provides feedback
   - **Submission**: Review and submit for moderation

     ![Publish Version](../../img/how-tos/entity-management/entity-versioning/publish-version.png)

**Unpublishing:**

Published agents can be unpublished at any time through the three-dot menu. This immediately removes the agent from Agent HUB.

<Note>
For detailed publishing workflow, validation requirements, moderation process, and troubleshooting, see [Agent Publishing](./agent-publishing).
</Note>
...
4. **Paste and share** the copied URL with your colleagues.
     ...
     ![Share](../../img/how-tos/entity-management/entity-versioning/share-version.png)


<Info title="Copy Link Behavior">
...
5. The recipient can **[import](./import-export)** the file into their own project.

     ![Export](../../img/how-tos/entity-management/entity-versioning/export-version.png)


<Info title="Export Behavior">
...
* Exported files can be imported back into ELITEA using the **[Import/Export](./import-export)** feature.
</Info>
...
* Each conversation maintains its own version selection independently
     ...
     ![Chat](../../img/how-tos/entity-management/entity-versioning/select-version-chat.png)
...
* Changing the default version in the source agent does **not** automatically update nested references - you must manually update the version selection in each parent agent/pipeline
     ...
     ![Agent](../../img/how-tos/entity-management/entity-versioning/select-version-agent.png)

<Tip title="Version Management in Nested Structures">
...
The **Compare Versions** feature allows you to view side-by-side differences between two versions of an agent or skill and selectively adopt changes from one version to the other.

### Access Compare Versions

To compare two versions:

1. Open any saved agent or skill with at least two versions.
2. Select the **three-dot menu (⋮)** in the entity tab bar.
3. Select **Compare Versions**.
     ![Compare Versions](../../img/how-tos/entity-management/entity-versioning/compare-version.png)
...
6. Select **Compare** to load both versions and enter the comparison wizard.
    ![Compare Versions Select](../../img/menus/agents/compare-version-select.png)
...
**For Agents:**
1. **Instructions**: View a diff of the instructions field showing added, removed, and unchanged text.
     ![Compare Instructions](../../img/how-tos/entity-management/entity-versioning/compare-instr.png)
2. **User Interaction**: Compare Welcome Message and Conversation Starters side-by-side.
     ![Compare User Interaction](../../img/how-tos/entity-management/entity-versioning/compare-Interaction.png)
3. **Tools & Skills**: See which toolkits, MCPs, agents, pipelines, and skills were added, removed, or kept the same between versions.
     ![Compare Tools](../../img/how-tos/entity-management/entity-versioning/compare-tools.png)
...
For each comparison step, you can:

* **View differences**: See what changed between the two versions with visual highlighting.
* **Adopt changes**: Copy changes from either version to the other by selecting the desired content.
* **Save individually**: Each editable section (Instructions, Welcome Message, Conversation Starters) has its own **Save** button. Changes are applied immediately when you save.

<Info title="Save Behavior">
* Changes are saved to the specific version being edited (left or right).
* You can save changes to one version without affecting the other.
* Each field is saved independently — you can save Instructions without saving other sections.
* Unsaved changes are preserved if you navigate between steps within the wizard.
</Info>

<Warning title="Unsaved Changes">
If you have unsaved changes when closing the Compare Versions wizard or changing versions, you will be prompted to confirm discarding those changes. Once discarded, changes cannot be recovered.
</Warning>

<Tip title="When to Use Compare Versions">
* **Merge parallel work**: When you have made changes in different versions and want to merge specific improvements without overwriting entire configurations.
* **Review version differences**: Compare a production version with a development version to see exactly what changed.
* **Selective adoption**: Adopt only specific changes (e.g., instructions) from one version to another without copying everything.
* **Version history review**: Understand how an agent or skill evolved between two versions.
</Tip>
...
4. The version is deleted and you're redirected to the default version (if set) or "base" version
     ...
     ![Delete](../../img/how-tos/entity-management/entity-versioning/delete-version-not-use.png)
...
7. You're redirected to the default version (if set) or "base" version
     ...
     ![Delete](../../img/how-tos/entity-management/entity-versioning/delete-version-in-use.png)


<Info title="Deletion Restrictions">
...
<Accordion title="Version Management Strategy">
...
<Accordion title="Workflow Recommendations">
...
<Accordion title="Cannot Delete Version">
...
<Accordion title="Version Name Validation Errors">
...
<Accordion title="Publishing Issues">
...
<Accordion title="Copy Link Not Working">
...
<Accordion title="Export Fails or Downloads Empty File">
...
- [Agents](../../menus/agents) - Complete guide to creating and configuring agents
 - [Pipelines](../../menus/pipelines) - Learn how to build and manage pipelines
 - [Import/Export Agents and Pipelines](./import-export) - Detailed guide to exporting and importing agents and pipelines
 - [ELITEA Glossary](../../home/glossary) - Definitions of key terms and concepts
</Info>