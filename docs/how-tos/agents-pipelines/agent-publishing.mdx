...
| **Category** | Required dropdown. Selects the Agent HUB category under which the published agent will appear. The list is loaded from the platform's configured agent categories for the current project. |
...
| **Continue** button | Enabled only when the version name is valid, not duplicate, a category is selected, and the checkbox is checked. Clicking it starts the automated validation. |
...
- **Publish** button (enabled only when a valid version name is entered and a category is selected)
- **Cancel** button
