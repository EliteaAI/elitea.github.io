...
4. Select **Publish** from the dropdown.
   ...
   ![Publish wizard opened via the three-dot menu](../../img/how-tos/agents-pipelines/agent-publishing/open-publish-menu.png)
...
This table describes the fields and actions on this step:
...
| **Category** | Required dropdown. Selects the Agent HUB category under which the published agent will appear. The list is loaded from the platform configured agent categories for the current project. |
...
| **Continue** button | Turned on only when the version name is valid, not duplicate, a category is selected, and the checkbox is checked. Clicking it starts the automated validation. |
| **Cancel** button | Closes the wizard without publishing. |

![Preparation step of the Publish wizard](../../img/how-tos/agents-pipelines/agent-publishing/publish-preparation-step.png)

<Info title="Why category matters">
...
Consumers of your published agent must provide their own credentials and configurations.
Exception: attached Skills and sub-agents are not stripped — their instructions are embedded in the published agent. Retained Skills are never listed as separate entries in the catalog.
...
The validation snapshot includes the agent content, nested sub-agent content, and any attached Skills retained by the agent. If any of that content changes after validation, the validation token becomes invalid and you must validate again.
...
<Tabs>
  <Tab title="Passed">
    ...
    ![Validation passed](../../img/how-tos/agents-pipelines/agent-publishing/validation-pass.png)
  </Tab>

  <Tab title="Not Passed">
    | Status | Message |
    |--------|---------|
    | <Badge color="red">FAIL</Badge> | "Sorry, your agent version does not meet all the necessary requirements. Follow summary details to fix the issues and try again." |
    ...
    - Return to the **Preparation** step to address the critical issues, then run validation again.
...
Once published, your agent appears in [ELITEA Catalog](../../menus/agents-studio), gets the **Published** badge on its card, and its version is marked as **published**.

<video controls width="100%" aria-label="Agent publishing full walkthrough.">
  <source src="../../img/how-tos/agents-pipelines/agent-publishing/agent-publishing-full.mp4" type="video/mp4" />
</video>
...
![Unpublish Agent dialog](../../img/how-tos/agents-pipelines/agent-publishing/unpublish-agent.png)
...
<AccordionGroup>
  ...
  <Accordion title="Validation fails with a transient AI error every attempt">
  ...
  <Accordion title="The Publish option is visible but disabled with a tooltip">
    **Cause:** The tooltip reads **"Publishing is blocked by platform policy"**, which means the platform `is_publish_blocked` setting is turned on and your project is not included in the allowed list.
  ...
  <Accordion title="An error appears on the Preparation step after clicking Continue">
  ...
  <Accordion title="Validation token expired or 'Agent was modified since validation'">
  ...
  <Accordion title="My attached Skill works in the published agent, but I cannot find it as a separate catalog item">
    **Cause:** This is expected behavior. Agent publishing retains attached Skills for the published agent runtime, but does not publish them as separately discoverable catalog entries.

    **Solution:** Publish the Skill separately through the Skill publishing flow if you want it to appear as an independent catalog item.
  </Accordion>

  <Accordion title="The agent publishes but shows a sub-agent warning">
    ...
    **Solution:** Open each sub-agent used within your agent configuration and publish them individually following the same three-step wizard. Once all sub-agents are published, users of the parent agent will have access to the complete workflow.
  </Accordion>

  <Accordion title="A published agent was unpublished and I received a notification">
  ...
  <Accordion title="Publishing fails with 'Maximum published versions reached'">
    ...
    **Solution:** Unpublish one or more earlier published versions of the same agent before publishing a new one.
  </Accordion>

  <Accordion title="Validation fails with 'LLM model is project-specific' or 'No LLM model configured'">
    ...
    **Solution:** Open the agent (or the affected sub-agent) in the editor, go to the **Model** settings, and select a model that is shared from the public project. After saving, re-run the publish wizard.
  </Accordion>
</AccordionGroup>
...
- [ELITEA Catalog](../../menus/agents-studio) — Browse and use published agents and skills.
