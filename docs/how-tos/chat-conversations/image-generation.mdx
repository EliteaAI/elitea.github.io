...
description: "Create, view, and manage AI-generated images in conversations and agents using image-capable models and tools."
---
...
* An image generation model configured and selected in your project (**Settings** → **Project** → **AI Providers**)
...
Image generation is available in two primary locations:
...
4. Toggle **Image creation** on to enable it.
5. Use the model selector to choose a model with image generation capabilities (e.g., GPT-5.2, GPT-5.1, DALL-E 3).
...
This option is available only to projects with the configured image generation provider toolkit. Without this toolkit, the option is silently omitted from the list.
Contact your administrator if you cannot see the option.
</Info>
...
4. In **TOOLS**, find **Modules**.
5. Find **Image creation**. If it is not immediately visible, select **Show all** to see the full list of modules.
6. Toggle **Image creation** on.
7. Use the model selector to choose a model with image generation capabilities (e.g., GPT-5.2, GPT-5.1, DALL-E 3).
8. Select **Save**.
...
Enabling **Image creation** at the agent level means every new chat that uses this agent has image generation available by default, and you should not enable it manually every time.
</Tip>
...
* Press Escape or click outside to close
     ...
     <img src="../../img/how-tos/chat-conversations/internal-tools/image-generation/image-full-size.gif" alt="Fuul size image" width="600" />
...
* You can download images from **Artifacts** at any time
...
* Hover over the generated image thumbnail in the chat to reveal the action buttons overlay
* Select **Delete** (trash icon, appears on hover)
...
* The dialog includes an **"Also delete from attachment storage"** checkbox (unchecked by default)
     ...
     * Check it to delete the file from the **`attachment`** bucket in Artifacts
* Select **Delete** to confirm deletion

     <img src="../../img/how-tos/chat-conversations/internal-tools/image-generation/image-delete.gif" alt="Fuul size image" width="600" />


<Info title="Storage and Deletion">
Generated images are automatically saved in **Artifacts** in the **`attachment`** bucket, inside a dedicated images folder. This folder is created automatically the first time you generate an image if it doesn't already exist.
</Info>
...
* **Modules** are hidden from the chat toolbar because the project has no modules available
* Network connectivity issues

**Solutions:**

1. Enable **Image creation** in **Modules**.
2. If the **Image creation** option is not visible in the popup, ask your administrator to add an image generation integration.
3. If **Modules** are missing from the chat toolbar entirely, contact your administrator to verify project configurations.
4. Refresh the page and try again.
</Accordion>

<Accordion title="Issue: Image Generation Model is Not Configured">
**Error Message:** "Image generation requested but no image generation model configured"

**Cause:** The module is enabled, but the selected model lacks image generation capabilities.
...
* **[Agent Configuration](../../menus/agents)** - Setting up agents with modules
