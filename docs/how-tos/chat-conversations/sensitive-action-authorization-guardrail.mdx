...
sidebarTitle: "Sensitive Action Guardrail"
description: "Review and approve sensitive tool actions before execution when the server-side authorization guardrail is enabled."
---
...
* **Dialog elements**
    ...
    | ![Block with Comment](../../img/how-tos/chat-conversations/sensitive-action/block-with-comment-button.png)| Expands an inline comment box on the authorization card so you can explain why the action is blocked and what the agent should do instead |
...
<Accordion title="The same tool keeps interrupting on every call despite being authorized">
