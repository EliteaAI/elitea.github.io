---
title: "Sensitive Action Authorization Guardrail"
---

<Warning title="Administrator-only configuration">
...
4. **User decision:** The user reviews the action label, parameters, and policy message, then clicks **Authorize**, **Block**, or **Block with Comment**.
5. **Resume or skip:**
    ...
    - **Block with Comment** → the tool is skipped, and the user's note is passed back to the agent as the blocked action's `denial_reason`.
...
<Info title="Need to add or remove a tool from the guardrail?">
...
![Authorization Card](../../img/how-tos/chat-conversations/sensitive-action/authorization-card.png)
...
* **Authorize**: Approves the action and resumes execution.

  ![Authorize](../../img/how-tos/chat-conversations/sensitive-action/authorize-button.png)

* **Block**: Rejects the action and skips the tool call entirely.

  ![Block](../../img/how-tos/chat-conversations/sensitive-action/block-button.png)

* **Block with Comment**: Expands an inline comment box on the authorization card so you can explain why the action is blocked and what the agent should do instead.

  ![Block with Comment](../../img/how-tos/chat-conversations/sensitive-action/block-with-comment-button.png)


<Info title="What happens when you block an action">
...
<Info title="What happens when you use Block with Comment">
When **Block with Comment** is available for the current interrupt, clicking it expands a multiline input directly on the authorization card. Your note is sent back to the runtime as the blocked action's `denial_reason`, allowing the agent to understand why the action was rejected and what alternative behavior you want.
</Info>
...
2. Click **Authorize** to proceed, **Block** to cancel immediately, or **Block with Comment** to cancel and provide guidance.
...
* **Block with Comment**
     ...
     ![Modal](../../img/how-tos/chat-conversations/sensitive-action/authorization-chat-block-comment.gif)
...
<Info title="Code nodes in pipelines">
...
* **Comment length limit** — Block-with-comment notes are capped at **2000 characters** in the UI and are trimmed before submission.
...
<Accordion title="Example 1: Protecting Destructive GitHub Operations">
...
<Accordion title="Example 2: Controlling Shell Command Execution">
...
<Accordion title="Example 3: Guarding Jira Project Deletion">
...
<Accordion title="Example 4: Wildcard Guard for Cross-Toolkit Operations">
...
<Accordion title="Can a user bypass the authorization dialog?">
...
<Accordion title="What if the agent calls two different sensitive tools in one message?">
...
<Accordion title="What is the difference between Block and Block with Comment?">
**Block** simply rejects the tool call.

**Block with Comment** also rejects the tool call, but sends your note back to the agent as structured feedback (`denial_reason`). This is useful when you want the agent to understand *why* the action was blocked or what it should do instead.
</Accordion>

<Accordion title="Does the authorization dialog expire?">
...
<Accordion title="Are pipeline runs affected?">
...
<Accordion title="What does the agent receive when a tool is blocked?">
...
<Accordion title="What does the agent receive when I use Block with Comment?">
The blocked tool result uses the same structure as a normal block, but the `denial_reason` field contains your comment instead of the default rejection message. The agent can use that note to choose a safer alternative or ask a follow-up question.
</Accordion>
...
<Accordion title="The authorization dialog never appears — the tool executes silently">
...
<Accordion title="The dialog appears but clicking Authorize does nothing">
...
<Accordion title="The tool is blocked even though I clicked Authorize">
...
<Accordion title="Security-sensitive parameters are not being masked">
...
* [Context Management](./context-management) — Manage conversation memory and history settings that affect what the agent retains between turns.
* [Toolkits](../../menus/toolkits) — Set up and manage toolkits to control which tools your agents can access, including those that trigger the guardrail.
* [Conversations](../../menus/chat) — Learn how to interact with agents in conversations, including how to respond to authorization dialogs.
* [Agents](../../menus/agents) — Configure agents with toolkits and understand how guardrail interrupts fit into the agent execution flow.
* [Pipelines](../../menus/pipelines) — Build multi-step pipelines and learn how authorization dialogs interrupt and resume pipeline node execution.
</Info>
