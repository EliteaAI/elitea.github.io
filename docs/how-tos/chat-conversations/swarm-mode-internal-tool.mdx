...
description: "Enable multiple agents to collaborate in a shared chat with handoffs and shared context."
---
...
<Tip title="When to Use Swarm Mode">
...
<Warning title="Swarm-enabled agents cannot be used as child agents">
...
| **Peer Agents** | Specialized agents | - Execute specific tasks using their tools<br />- Access full shared conversation history<br />- Hand control back to the entry agent when done<br />- Can also hand off to other peers |
...
4. **Specialist Work:** Peer agent executes the task using its specialized tools and knowledge
...
<Note>
Only **one handoff can be processed per turn**. If the entry agent emits multiple handoff tool calls in a single response, they are executed in order (not concurrently).
</Note>
...
Enable Swarm Mode for collaborative multi-agent chats.

1. Go to your chat.
2. In the chat toolbar, select **+**.
3. Select **Modules** → **Swarm Mode** in the list.
5. Toggle **Swarm Mode** on to enable it.

<img src="../../img/how-tos/chat-conversations/internal-tools/swarm-mode/euch_swarm_on_chat.png" alt="Enable Swarm Mode in chat"/>

<Info title="Configuration Persistence">
The Swarm Mode setting persists for the duration of your chat session. You can toggle it on/off at any time. However, Swarm Mode requires child agents to be configured in the agent's toolkits to function.
</Info>
...
5. Find **Swarm Mode**. If it is not immediately visible, select **Show all** to see the full list of modules.
6. Toggle **Swarm Mode** on.
7. Select **Save**.

     <img src="../../img/how-tos/chat-conversations/internal-tools/swarm-mode/euch_swarm-for-agent.png" alt="Enable Swarm Mode for agents" width="400" />

<Tip title="Configure Child Agents First">
...
<Info title="Example Interaction">
...
<Info title="Handoff Tools Are Automatic">
You don't need to manually create handoff tools. When Swarm Mode is enabled, the system automatically generates `transfer_to_[agent_name]` tools for every peer combination. Each agent also receives a `transfer_to_main_agent` tool to hand control back to the entry agent.
</Info>
</Info>
...
Swarm Mode works well with other modules like Smart Tools Selection (for token optimization), Data Analysis (for specialist data agents), and Python Sandbox (for code execution specialists).
</Accordion>
...
<Info title="Swarm Mode Requires Child Agents">
...
* **[Smart Tools Selection](./smart-tools-selection-internal-tool)** - Optimize token usage with many toolkits
...
* **[Toolkits Overview](../../menus/toolkits)** - Understanding and configuring toolkits
</Info>
