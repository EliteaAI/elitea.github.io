...
<CardGroup cols={3}>
  ...
  <Card title="Modules" icon="toolbox">
...
<Steps>
  <Step title="Access the Agent Creation Canvas">
    ...
    3. Hover over **Agents** to expand the submenu.
    4. Click **Create New Agent** at the bottom of the submenu.
  ...
  <Step title="Configure Agent">
    ...
    **GENERAL**
    ...
    <Tip title="Build with AI">
    Instead of filling in the form manually, click the **✨ Build with AI** button in the **GENERAL** section header. Describe what the agent should do — its goal, tasks, and preferred tone — then click **Generate**. ELITEA drafts a **Name**, **Description**, **Instructions**, **Welcome Message**, and **Conversation Starters** for you. All generated fields are fully editable before you save.
    </Tip>
    ...
    **INSTRUCTIONS**
    ...
    <Tip>
    The Instructions field supports two shorthand syntaxes:
    - **Variables**: Use `{{variable_name}}` to define template placeholders. They are automatically detected and added to the **VARIABLES** section, where you can set their default values.
    - **Mentions**: Type `/` to open a suggestion dropdown listing all configured toolkits, MCPs, agents, and pipelines. For toolkits and MCPs, you can optionally select a specific tool to produce a `/Name/ToolName` mention. Type `~` to open a suggestion dropdown listing attached Skills. Inserted mentions are highlighted in the field.
    </Tip>
    ...
    **ADVANCED** (optional)
    ...
    - **Ignore Project Context**: When enabled, the agent does not receive the project-level [Project Context](../../menus/settings/project-context) that is otherwise injected into every AI interaction. Enable this when the agent should rely solely on its own instructions.
  ...
  <Step title="Save Initial Configuration">
    Click **Save** to create your agent. After saving, the canvas transitions to **edit mode** where the full configuration interface becomes available:
    ...
    <CardGroup cols={3}>
      ...
      <Card title="Modules" icon="toolbox">
        Built-in capability toggle cards appear in **TOOLS**, ready to enable.
      </Card>
    </CardGroup>
...
## Build Agent with AI from Canvas

You can create an agent from Canvas without filling every field manually. **Build with AI** is available directly inside the Agent Canvas and generates a draft configuration from a plain-language description.

<CardGroup cols={2}>
  <Card title="Canvas-Native Flow" icon="wand-magic-sparkles">
    Start from the same **Create New Agent** canvas and generate the first draft without leaving chat.
  </Card>
  <Card title="Generated Draft" icon="file-lines">
    ELITEA drafts the **Name**, **Description**, **Instructions**, **Welcome Message**, and **Conversation Starters** for review.
  </Card>
  <Card title="Editable Before Save" icon="pen-to-square">
    All generated values remain editable before you save the new agent from Canvas.
  </Card>
  <Card title="Project-Aware Suggestions" icon="link">
    The generated draft can align with the resources already available in your project.
  </Card>
</CardGroup>

**Use Build with AI in Canvas**

1. Open the Agent Canvas from **Chat** → **`+`** → **Agents** → **Create New Agent**.
2. In the **GENERAL** section header, click **Build with AI**.
3. Describe what the agent should do, including its goal, tasks, and preferred tone.
4. Click **Generate**.
5. Review the generated draft fields and edit anything you want to change.
6. Click **Save** to create the agent from Canvas.

<Tip title="Best Results">
Include the agent's purpose, expected tasks, and preferred tone in your prompt. More specific prompts usually produce a better first draft.
</Tip>
...
<Info title="Need the Full Workflow?">
For the complete Build with AI flow, including generated fields, review behavior, and related examples, see [**Build an Agent with AI**](../agents-pipelines/build-agent-with-ai).
</Info>
...
In the **TOOLS** section, enhance your agent's capabilities by adding integrations and enabling modules.
...
**MODULES** display built-in capabilities as toggle cards. Enable these tools to extend your agent's functionality without needing external integrations:

<img src="../../img/how-tos/chat-conversations/canvas-agent/euch_modules.png" style={{width: "500px", height: "auto"}} alt="Modules" />
...
<CardGroup cols={3}>
  ...
  <Card title="Agents & Pipeline Builder" icon="plug" href="./elitea-mcp-tools-internal-tool">
...
The **SKILLS** section (available in team and personal projects) lets you attach reusable instruction sets to the agent version.

<Info>
A maximum of **5 Skills** can be attached to a single agent version. The **SKILLS** section is hidden when the Public project is selected. The **+ Skill** button is disabled until the agent has been saved at least once.
</Info>

1. Scroll to the **SKILLS** accordion in the left panel.
2. Click **`+ Skill`** to open the skill picker and search by name, then click to attach. The Skill is attached using its `base` version by default.
3. To change the version, click the version name on the Skill card and select from the dropdown.
4. To remove a Skill, hover over the card and click the **Remove skill** (trash) icon.

![Agent Advanced Configuration](../../img/how-tos/chat-conversations/canvas-agent/canvas-agents-skills.gif)


<Info>
For full details on creating and managing Skills, see the [**Skills**](../../menus/skills) documentation.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Tools and Integrations" icon="toolbox">
    Add or remove toolkits, MCP connections, nested agents, and nested pipelines. Toggle modules on or off as needed.
  </Card>
</CardGroup>


After making changes, click **Save** to apply them. **Discard** resets the form to the last saved state. Clicking **×** with unsaved changes triggers a confirmation dialog.
