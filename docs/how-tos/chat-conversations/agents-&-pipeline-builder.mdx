...
**Agent & Pipeline Builder** is a built-in Module that connects a conversation agent to ELITEA's own **Model Context Protocol (MCP) server**. Once enabled, the agent can create and update agents and pipelines directly from chat, while also discovering and calling other agents and toolkit operations that have been explicitly made available through MCP — all from within a single conversation, without the user needing to pre-wire every dependency upfront.
...
<Tip title="When to Use Agent & Pipeline Builder">
...
- **MCP Enabled**: MCP features must be enabled on the platform. If the **Agent & Pipeline Builder** toggle does not appear in the Modules list, contact your Elitea administrator.
...
## Enabling Agent & Pipeline Builder

You can enable Agent & Pipeline Builder in three ways: as a default setting for all new chats and agents, for a specific conversation, or in an individual agent configuration.

<Note>
If **Agent & Pipeline Builder** does not appear in the Modules list, the MCP feature may be disabled at the platform level. Contact your Elitea administrator.
</Note>

<Tip>
**Recommended:** Enable the **Ask User** module alongside Agent & Pipeline Builder for the best experience. When enabled, the conversation can ask structured clarifying questions with interactive options if your agent or pipeline creation request is missing information. Without Ask User, the conversation can only request information through normal chat responses.
</Tip>
...
3. Find the **Agent & Pipeline Builder** row under default modules.
4. Toggle **Chats** on to enable Agent & Pipeline Builder by default for all new conversations.
5. Toggle **Agents** on to enable Agent & Pipeline Builder by default for all new agents.
6. Click **Save**.

     <img src="../../img/how-tos/chat-conversations/internal-tools/agent-&-pipeline-builder/enable-general.png" alt="Enable Agent & Pipeline Builder in Project Settings" width="350" />

New chats and agents in this project will have Agent & Pipeline Builder enabled based on your toggle settings.
...
4. Find **Agent & Pipeline Builder** in the list and toggle it on.
...
4. Find the **Agent & Pipeline Builder** toggle (click **Show all** if needed).
...
New conversations with this agent will have Agent & Pipeline Builder enabled by default.
...
You can use Agent & Pipeline Builder to create a new agent or pipeline directly from a conversation.
...
- [Ask User Module](../modules/ask-user) - Recommended module to use with Agent & Pipeline Builder for structured clarification questions
