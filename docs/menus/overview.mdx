...
1. **Sidebar**: The sidebar drawer allows users to switch among different menus such as Chat, Agents, Pipelines, Credentials, Toolkits, MCPs, Artifacts, Applications and Agent Studio.
...
13. **Applications**: Menu, providing access to purpose-built application integrations such as Wikis and Inventory. Configure and manage deployable solutions that extend ELITEA with specialized, domain-focused capabilities.
14. **Agent Studio**: Special button located in the bottom section of the sidebar, providing quick access to the centralized shared library for discovering and using community-published agents. Browse by categories, search by keywords, and instantly start conversations with pre-built AI assistants.
15. **Settings**: Menu, here, you can configure various project and user-specific settings.
16. **Resources**: Special button located in the bottom section of the sidebar, providing access to platform documentation, release notes, video walkthroughs, tutorials, and interactive tours. This menu replaces the previous **Tips** menu.
17. **Notifications**: Notification bell allowing users to get notified about various events such as agent publishing status within ELITEA.
18. **User Profile**: User account section displaying the current user's name and avatar, accessible at the bottom of the sidebar for account management.
19. **Status**: Status indicators and filters showing the current state of agents and other entities (e.g., "All", "Draft", "Published" states).
20. **View Modes**: Cards are displayed in responsive grid layouts that automatically adjust based on screen size and content.
21. **Search**: A Search box available to find agents, pipelines, toolkits, and credentials by their names and descriptions.
    !!! info "Scope" The **Search** functionality operates within the selected menu and is not universal across the entire platform.
22. **Tags**: This section displays the tags (categories) associated with the content being viewed. The tags vary depending on the selected menu.
23. **Teammates**: Shows team members and collaborators who are working on the current project.
24. **Feedback**: Icon located in the bottom left of the screen, allowing users to rate and provide feedback about the ELITEA platform.
