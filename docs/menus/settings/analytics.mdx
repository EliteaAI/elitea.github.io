---
title: "Analytics"
description: "The Analytics feature in ELITEA provides detailed, real-time visibility into how your team uses the platform — from AI model invocations and tool executions to "
---
...
<Info title="SUGGESTIONS">
Add that the current analytics API queries use the same 5-minute cache lifetime across overview, list, detail, and cost analytics requests.
</Info>

<Warning title="Project Member Requirement">
...
<Info title="SUGGESTIONS">
Add that the current page also includes a dedicated **Costs** tab.
</Info>
...
<Info title="SUGGESTIONS">
Update this description to match the current UI. The current tab bar contains seven tabs: **Overview**, **Costs**, **Agents & Pipelines**, **Tools**, **Users**, **Health**, and **Guide**.
</Info>
...
<Info title="SUGGESTIONS">
Update this sentence to say that the dashboard is divided into seven tabs.
</Info>
...
<Info title="SUGGESTIONS">
Add that the current UI fetches the overview analytics payload for both the **Overview** and **Health** tabs.
</Info>
...
<Info title="SUGGESTIONS">
Add a new section for the **Costs** tab.

Suggested content:

- The **Costs** tab shows estimated LLM spend for the selected date range.
- ELITEA calculates these values from a local model-price table, so provider invoices may differ.
- The tab includes KPI cards for **Total Cost**, **Total Tokens**, **Input Tokens**, **Output Tokens**, and **Avg Cost / Call**.
- The tab includes these breakdowns:
     - **Cost by Model**
     - **Daily Cost Trend**
     - **Cost by Agent & Pipeline**
     - **Cost by User**
- If no cost data is available for the selected date range, the tab shows empty-state messages for the affected charts or tables.
</Info>
...
<Info title="SUGGESTIONS">
Rename this section to **Agents & Pipelines Tab** to match the current UI label.
</Info>
...
<Info title="SUGGESTIONS">
Add that the current UI reuses the same overview analytics request for the **Health** tab.
</Info>
...
<Tip title="First Time Using Analytics?">
...
<Info title="SUGGESTIONS">
Add these current limitations:

- **Costs are estimates** — The **Costs** tab uses a local model-price table, so provider invoices may differ.
- **Costs uses a separate request** — The **Costs** tab loads from a dedicated cost analytics endpoint.
- **Seven-tab layout** — The current Analytics page includes a dedicated **Costs** tab.
</Info>
...
<Info title="Related Pages">
- [Users](./users) — Manage project members whose activity is tracked in Analytics.
- [Toolkits](../toolkits) — Configure and manage the tools that appear in the Tools tab.
- [Agents](../agents) — Create and manage the agents whose usage is tracked in the Agents tab.
</Info>
