...
| **Multiple view modes** | Rendering modes vary by file type—see **View Modes by File Type** below. |
...
| File Type | View Modes |
|-----------|------------|
| **Text and code files** | Syntax highlighting, line numbers, and proper formatting for 50+ supported programming languages and file types. |
| **HTML files (.html)** | Toggle between **Raw** (source) and **Preview** modes. Preview mode renders the file and executes its embedded JavaScript in a sandboxed frame. |
...
<Note>
**HTML Preview & JavaScript Execution**

HTML files open in **Preview** mode by default, rendering the file in a sandboxed frame and automatically **executing any embedded JavaScript** — there is no separate "Run" button and no confirmation prompt. Toggle to **Raw** mode to view or edit the source without executing anything.

For safety, the preview frame is sandboxed and restricted:

* No network access—`fetch`/`XHR` requests and any outbound connections are blocked.
* External `<script src="...">` tags and embedded `<iframe>`, `<object>`, or `<embed>` elements are stripped before rendering; a warning banner reports how many were removed.
* Only `data:` and `blob:` images are allowed; other external resources do not load.
* Links inside the preview never navigate—selecting one opens an **External Link** confirmation dialog showing the URL instead.

This makes the HTML preview suitable for viewing dynamic, self-contained content (for example, interactive charts or small demos), but it is not a general-purpose browser sandbox.
</Note>
