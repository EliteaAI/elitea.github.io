...
![Agents-Menu\_Interface](../img/menus/agents/agents-view.gif)
...
![Agents-Menu\_Interface](../img/menus/agents/agents-filter.gif)
...
![Agents-Menu\_Interface](../img/menus/agents/agents-pin.gif)
...
![Agents-Menu\_Create](../img/menus/agents/agents-create.gif)
...
The Instructions field supports `/` mentions to reference tools added to the agent directly in the instruction text. Inserted mentions are highlighted in the field so you can see at a glance which tools are referenced.
...
1. Type `/` anywhere in the Instructions field to open the suggestion dropdown.
2. The dropdown lists all **toolkits, MCPs, agents, and pipelines** that are added to the agent and have no configuration errors.
3. Start typing after `/` to filter the list by name.
...
<Warning>
Only tools that are already added to the agent's **Toolkits** section and have no validation errors appear in the suggestion list. Add a toolkit to the agent first before mentioning it in instructions.
</Warning>
...
2. **Adjust Model Settings** (Optional): Click the **Model Settings** icon (⚙️) next to the model selector to fine-tune the response generation. The settings vary depending on the selected model:
   ...
   **Max Completion Tokens** Limits the maximum length of AI responses measured in tokens (roughly 4 characters per token).(All Models):
   ...
   ![Agents-Model- Settings](../img/menus/agents/agent-model_settings.gif)
...
You can publish your agents to [Agent HUB](./agents-studio), a shared library accessible to all users in your ELITEA installation. The publishing wizard uses AI-powered automated validation to check your agent before publishing.

To publish an agent:

1. Open the agent and select its version you want to publish.
2. Click the **three-dot menu (⋮)** and select **Publish**.
...
5. Click **Publish** to submit. On success, the version is immediately live in Agent HUB.

![Agents-Publish](../img/menus/agents/agents-publishing.gif)

What happens after publishing:

- The version status updates to **Published**.
- Community users can find and use the agent in Agent HUB without needing the project access.
- You can unpublish at any time using the **Unpublish** option in the three-dot menu.

<Warning title="Publishing permissions">
Only users with publishing permissions (`models.applications.publish.post`) can publish agents.
</Warning>
