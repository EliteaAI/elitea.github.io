...
**Chat** is the central hub where all platform capabilities come together. It provides a conversational interface where you can interact with AI models, agents, pipelines, skills, modules, toolkits and MCP servers — all in one place, using natural language.
...
<CardGroup cols={3}>
  <Card title="Public and Private Conversations" icon="users">
  ...
  <Card title="Participants" icon="user-group">
    Add various participants to the conversation, including other users in public conversations, modules, agents, pipelines, toolkits and MCPs.
  </Card>
  <Card title="Create and Edit Entities" icon="pen-to-square">
    Create and edit entities directly from the conversation without leaving the chat interface.
  </Card>
  ...
  <Card title="Managing Conversations" icon="sliders">
    Save conversations, pin the most important ones, delete and clean content.
  </Card>
  ...
  <Card title="Playback" icon="circle-play">
    Move through the playback process or stop the conversation by simulating the current conversation without any engagement with models.
  </Card>
</CardGroup>
...
2. Click the **+ Conversation** button at the top of the sidebar, or select an existing conversation and a welcome screen appears.
3. The **New Conversation View** is shown: a welcome screen with a greeting (*Hello, \{firstName\}! What can I do for you today?*) and the chat input centered on screen.
4. Optionally add participants before sending — use the **`+`** button in the input toolbar or type **`#`** to search (see [Participants](#participants) below).
5. Type your first message in the input field.
6. Press **Send** (or press Enter).
7. The conversation is created and appears in the **CONVERSATIONS** sidebar.
8. The conversation name is automatically generated based on your message content. During name generation (1-2 seconds), you'll see a loader icon with "Naming" text next to the conversation item.
...
7. Added users will appear in the **Users** section of the **PARTICIPANTS** panel. Hover over user avatars in the participant list or type **@username** in the chat input to mention and notify teammates in the conversation. To mention everyone in the conversation, select the **All Users** option.
   <img src="../img/menus/chat/add-user.gif" alt="Add User" />        

<Info title="Note">
* **Invite Users** is only available in team projects. It is not shown in personal (private) projects.
...
Modules provide built-in capabilities that enhance your chats without requiring external integrations. These modules can be enabled directly from the chat interface or configured as part of an agent's default setup.

**Available Modules:**

<CardGroup cols={2}>
  <Card title="Python Sandbox" type="note" icon="python" href="../how-tos/chat-conversations/python-sandbox-internal-tool">
    Execute Python code securely in chats using Pyodide (Python compiled to WebAssembly). Useful for calculations, data processing, testing algorithms, and generating visualizations.
  </Card>
  ...
  <Card title="Planner" type="note" icon="list-check" href="../how-tos/chat-conversations/planner-internal-tool">
    Create, manage, and track tasks and action items directly within chats. Set priorities, due dates, and monitor task progress without switching to external task management tools.
  </Card>
  ...
  <Card title="Attachments" type="note" icon="paperclip" href="../how-tos/chat-conversations/attach-files">
    Attach files and images to chats for AI-powered analysis. Files are automatically stored in the default `attachments` artifact bucket.
  </Card>
  ...
  <Card title="Agent & Pipeline Builder" type="note" icon="server" href="../how-tos/chat-conversations/agents-&-pipeline-builder">
    Enable built-in ELITEA MCP tools for managing agents, pipelines, chat, and toolkits directly from within chats — without switching to a separate configuration screen.
  </Card>
</CardGroup>
...
You can also enable modules as part of an agent's default configuration in the **TOOLKITS** section. This makes the tools available in all new chas using that agent.
</Tip>

### Attachments in Chats

Attach files and images directly to chats for AI-powered analysis. This feature enables multimodal interactions where AI can process visual content, documents, and data files alongside text-based queries.
...
<CardGroup cols={3}>
  ...
  <Card title="Data Files" icon="table">
    Upload CSV, JSON, and other data formats for analysis and processing.
  </Card>
  <Card title="Multiple Upload Methods" icon="upload">
    Click to browse, drag-and-drop onto the input area, or paste from clipboard.
  </Card>
  <Card title="Centralized Storage" icon="box-archive">
    All attachments are automatically stored in the default `attachments` Artifact bucket — no manual configuration required.
  </Card>
</CardGroup>
...
The **Attach Files** item in the `+` menu shows a counter (*N left*) reflecting how many more files can still be attached in the current message.
...
7. Click the **✕** button in the voicewave pill to exit Speaking Mode at any time.
   ...
   ![Speaking Mode](../img/menus/chat/speaking-mode.gif)
