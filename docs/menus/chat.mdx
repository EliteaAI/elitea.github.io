...
<img src="../img/menus/chat/chat.png" alt="The Chat page with an ongoing conversation." />
...
<img src="../img/menus/chat/chat_page-structure.png" alt="The Chat page with the CHATS sidebar, PARTICIPANTS panel, Context Budget widget, and main chat area labeled." />
...
The main chat area contains the conversation header, message history, and message input toolbar.
...
<table>
  ...
  <tbody>
    <tr><td>1</td><td><strong>Message input field</strong></td><td>Enter your message here.</td></tr>
...
Type your message in the input field, then press **Enter** to send it, or **Shift+Enter** to insert a line break without sending. Type `@` followed by a name to mention a user, or `#` followed by a name to mention an agent, pipeline, toolkit or MCP.

While the AI is generating a response, the input field is disabled — select **Stop generating** to interrupt the response early. Stopping restores your last message to the input field so you can edit and resend it.

<Note>
If your ELITEA administrator has [turned on mid-turn input for your project](./settings/general#chat), you can keep typing and send additional messages while the AI is still generating a response, up to 4000 characters per message. The message is injected into the running turn instead of starting a new one.

<img src="../img/menus/chat/chat_min-turn-input.png" alt="The chat input field with a Send to the running agent button shown during response generation." />
...
<img src="../img/menus/chat/chat_model-selector.png" width="350" alt="The model selector dropdown at the bottom of the chat." />
...
<img src="../img/menus/chat/chat_model-labels.png" width="200" alt="A model name in the selector with an image analysis or reasoning capability icon." />
...
<Tabs>
  <Tab title="Reasoning Models">
    ...
    <img src="../img/menus/chat/chat_model-sets_reasoning.png" width="300" alt="The reasoning level setting in model settings for a reasoning model." />
  </Tab>
  <Tab title="Standard Models">
    ...
    <img src="../img/menus/chat/chat_model-sets_standard.png" width="300" alt="The creativity level setting in model settings for a standard model." />
  </Tab>
</Tabs>
...
<CardGroup cols={3}>
  ...
  <Card title="Data files" icon="table">
    Upload CSV, JSON, and other data formats for data processing.
  </Card>
</CardGroup>
...
<CardGroup cols={3}>
  <Card title="Image Creation" icon="image" href="../how-tos/chat-conversations/image-generation" />
  <Card title="Data Analysis" icon="chart-bar" href="../how-tos/chat-conversations/data-analysis-internal-tool" />
  <Card title="Agent & Pipeline Builder" icon="sitemap" href="../how-tos/chat-conversations/agents-&-pipeline-builder" />
  <Card title="Skill Builder" icon="wand-magic-sparkles" href="../how-tos/modules/skill-builder" />
  <Card title="Project Context Builder" icon="folder-tree" href="../how-tos/modules/project-context-builder" />
  <Card title="Ask User" icon="circle-question" href="../how-tos/modules/ask-user" />
  <Card title="Planner" icon="list-check" href="../how-tos/chat-conversations/planner-internal-tool" />
  <Card title="Python Sandbox" icon="terminal" href="../how-tos/chat-conversations/python-sandbox-internal-tool" />
  <Card title="Swarm Mode" icon="people-group" href="../how-tos/chat-conversations/swarm-mode-internal-tool" />
  <Card title="Smart Tools Selection" icon="bolt" href="../how-tos/chat-conversations/smart-tools-selection-internal-tool" />
</CardGroup>

Default modules are configured by project administrators in **Settings**,  [General > Default Modules](./settings/general#default-modules).

All team members can turn a module on or off for their own chats or agents at any time, regardless of the project default.

<Info>
For detailed information about modules, see [Modules Overview](../how-tos/modules/modules).
</Info>
...
<img src="../img/menus/chat/chat_chats-sidebar.png" width="250" alt="The CHATS sidebar with conversations grouped into Today, Yesterday, This Week, and Older." />
...
Unlike conversation actions, all **folder actions are restricted to the folder's creator** and, in team projects, also require the corresponding folder permission:
...
<img src="../img/menus/chat/chat_actions_folder.png" width="250" alt="The folder contextual menu with Rename, Pin on top, and Delete Folder options." />

### PARTICIPANTS Panel

The **PARTICIPANTS** panel lists everyone and everything taking part in the current conversation — agents, pipelines, toolkits and MCPs, as well as users in team projects.
...
In team projects, you can invite specific members of your team to collaborate in the conversation. These users are invited from the **+** button in the message input toolbar and are listed in the **PARTICIPANTS** panel as Users:

<img src="../img/menus/chat/chat_participants_users.png" width="250" alt="The PARTICIPANTS panel showing users invited to a team project conversation." />

<Note>
Users can only be invited to a conversation in team projects. Private projects have no other members, so this section does not apply there.
</Note>

Unlike Agents, Pipelines, Toolkits and MCPs, Users are not added to a conversation — they already have access through project membership. You can invite users for quicker and easier mentioning.

Once a user is added to the conversation, you can mention them by hovering over their card and selecting **Mention**, or by typing `@` followed by their name in the chat input. This inserts an `@username` mention and addresses that user directly.

#### Context Budget

The **Context Budget** widget displays real-time token usage metrics and allows you to configure how the system manages conversation context as it approaches model token limits.
...
The widget appears in the bottom-left area of the **PARTICIPANTS** panel after you send the first message in a conversation.
...
* **Collapsed view**: Shows a simple line indicating the token usage status (green for normal, orange for high usage)

  <img src="../img/menus/chat/chat_context-budget_collapsed.png" alt="The Context Budget widget in collapsed view, showing a token usage status line." />

* **Compact view**: Displays pruning strategy, message count, summaries count, and an expand button

  <img src="../img/menus/chat/chat_context-budget_compact.png" width="250" alt="The Context Budget widget in compact view, showing pruning strategy, message count, and summaries count." />

* **Configuration view**: Allows you to set context management parameters, including max context tokens, message preservation, pruning strategy, and summarization settings

  <img src="../img/menus/chat/chat_context-budget_configuration.png" width="450" alt="The Context Budget widget in configuration view, showing context management parameters." />

**Configuration Parameters**

In the **Context Budget** widget, you can configure:
...
Visibility and access rules for conversations and folders depend on your project type — private or team.
...
* **Conversations**: A conversation is only visible to you if you are added to it as a participant (= conversation member).
...
Beyond plain text responses, ELITEA can ask you clarifying questions, suggest follow-up messages, and open entities directly from a chat response.

### Clarifying Questions

When you turn on the **Ask User** module for your conversation in chat or an agent, LLM can pause and ask you one or more clarifying questions instead of guessing. These clarifying questions are shown as a card in the chat:

* Each question appears one at a time, with a stepper showing your position (e.g. `1/4`) and arrows to move between questions.
* You can select one of the provided options, or use the free-text field to enter your own answer.
* Select **Continue** to submit your answer and move to the next question, or **Skip** to leave a question unanswered.

Your answers are sent directly back to the conversation or agent to resume its run, not as a new chat message.
...
<Note>
**Ask User** must be turned on for the conversation or agent you work with. You can check whether it is enabled by default for your project in **Settings** > **General**.
</Note>

<Info>
For detailed information, see [Ask User](../how-tos/modules/ask-user.mdx).
</Info>

### Follow-up Suggestions

After a response, ELITEA can automatically suggest possible next messages, shown as clickable chips below the last message:

<img src="../img/menus/chat/chat_follow-up-suggestions.png" alt="Follow-up suggestion chips shown below the last message in the chat." />

Select a suggestion chip to insert its text into the message input field. The suggestion is not sent automatically — review or edit it, then send it like any other message.

<Note>
Follow-up suggestions are controlled by your ELITEA administrator and are only generated in live chat sessions, not in scheduled or webhook-triggered runs.
</Note>

### Conversation Starters

When you add a participant to a conversation, the configured conversation starter for that participant will be automatically displayed in the chat. This will provide you with the immediate context and guide you on how to interact with each participant.
...
### Chat-Generated Entities

When an agent or pipeline generates an **Agent**, **Pipeline**, **Toolkit**, **MCP**, **Skill** or **Project Context** in a chat message, ELITEA automatically opens it in the Canvas side-by-side with the chat main area:
...
All entities open in the matching editor with the generated content pre-filled. You can review and refine your agent, skill or other generated entity before saving it to your project.

If ELITEA generated several entities in a single message, they are opened in separate tabs in the Canvas area. You can switch between them using the tabs at the top of the Canvas:

<video src="../img/menus/chat/chat_canvas-two-tabs.mp4" controls></video>

Besides opening in Canvas, generated entities are displayed as chips in the chat message. You can interact with them directly from the chat:

* Hover over the chip to reveal the **Edit** and **Delete** icons.
* Select **Edit** to open the entity in its matching editor (Agent, Pipeline, Toolkit, Skill or Project Context editor) in a Canvas-side tab, where you can review and refine it before saving.
* Select **Delete** to discard the generated entity without creating it.

<Info>
MCPs do not have a separate chip type — a generated MCP is a Toolkit definition, shown with the MCP icon on its chip and opened in the same Toolkit editor.
</Info>
...
- **Microphone icon is hidden**: No server-side ASR model is configured and your browser does not support the Web Speech API. Check your browser compatibility or ask your administrator to configure an ASR model.
...
* [Sensitive Action Authorization Guardrail](../how-tos/chat-conversations/sensitive-action-authorization-guardrail)
