...
The pipeline creation flow follows a **two-phase approach**: first you provide the pipeline metadata and save, then the full editor environment — including the Flow/YAML designer, Toolkits, and the embedded test chat — becomes available.
...
- **Left panel** — the configuration form with all settings sections (General, Toolkits, Variables, Welcome Message, Conversation Starters, Advanced, Information)
...
<Frame caption="If a toolkit added to your pipeline uses shared credentials that require each user to supply their own private copy, a **Credential setup required** warning banner will appear on the tool card. Click the **Create a credential** link in the banner to create a matching private credential in your Personal workspace. See [Shared Toolkit Credentials](./toolkits#shared-toolkit-credentials) for details.">
  <img src="../img/menus/pipeline/credentials-warning-pipeline.png" alt="Credential setup required banner" />
</Frame>
