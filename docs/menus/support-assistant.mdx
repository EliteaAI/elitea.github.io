...
<CardGroup cols={2}>
  ...
  <Card title="Mermaid & Image Rendering" icon="diagram-project">
    Mermaid code blocks render as visual diagrams, and inline images or GIFs render directly in the conversation with click-to-expand preview.
  </Card>
...
<Info title="Stopping a Response">
While the assistant is generating a response, a **Stop** button appears in place of **Send**. Selecting it interrupts generation immediately — any partial output already produced remains visible in the conversation, followed by a message confirming that generation was stopped.
</Info>

## Response Rendering

The Support Assistant uses the same rich message rendering approach as the main ELITEA Chat experience for common response content.

**Rendering parity includes:**

- **Mermaid diagrams**: fenced code blocks marked as `mermaid` are rendered as visual diagrams instead of being shown as raw code.
- **Inline images and GIFs**: both Markdown image syntax and inline HTML `<img>` tags render directly in the assistant conversation.
- **Expanded image preview**: selecting an inline image opens a larger preview overlay for easier inspection.
- **Markdown formatting**: tables, lists, headings, inline code, and standard fenced code blocks render in formatted form.

<Info title="Mermaid and HTML Image Support">
Support Assistant responses support Mermaid diagram rendering and inline image rendering through the same Markdown pipeline used by the assistant widget, including GitHub-flavored Markdown and embedded raw HTML image tags.
</Info>
...
<Accordion title="Mermaid Diagram or Inline Image Does Not Render Correctly">
Support Assistant renders Mermaid diagrams and inline images directly in the conversation. If rendering fails:

- Verify the Mermaid block uses valid fenced syntax such as ` ```mermaid `.
- Check that inline HTML image tags use a valid `src` value.
- If a Mermaid diagram contains invalid syntax, the assistant may fall back to showing the raw Mermaid code block.
- Reload the page and reopen the assistant if the message was streamed while the widget was still initializing.
</Accordion>
