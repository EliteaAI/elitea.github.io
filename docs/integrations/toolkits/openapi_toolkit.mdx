---
title: "OpenAPI"
description: "Connect ELITEA to any OpenAPI-compatible service — import a spec and turn any REST API into callable tools for your agents."
---
...
**Additional Static Headers (optional, any authentication type):**

*   One or more custom header name/value pairs to send with every request (for example, an API gateway key or tenant ID alongside your primary authentication)
...
<Warning title="Important for Release 2.0.0 B2 Users">
...
**For existing toolkit migration instructions**, see the [OpenAPI Toolkit Migration Guide](../../migration/v2.0.1/openapi-toolkit-migration).
</Warning>
...
3. **Select OpenAPI Configuration:** Choose **OpenAPI Configuration** as the credential type.
      ![OpenAPI Credential Type](../../img/integrations/toolkits/openapi/openapi-cred-type.png)
4. **Configure Credential Details:**

     | Field | Required | Description |
     |-------|----------|-------------|
     | **Display Name** | Required | Descriptive name for this credential (e.g., *"GitHub API - Production"*, *"Petstore API Key"*) |
     | **ID** | Required | Unique identifier for the credential. Auto-populated from the Display Name |
     | **Authentication Method** | Required | Choose based on your API requirements: **API Key Authentication** — for Bearer tokens, Basic auth, or Custom headers · **OAuth (Delegated)** — for delegated OAuth flows that use an OAuth discovery endpoint and interactive login · **OAuth (Client Credentials)** — for machine-to-machine OAuth flows using client credentials and token exchange · **Anonymous** — for public APIs without authentication requirements |
     | **Additional Headers** | Optional | One or more static header name/value pairs sent with every request, regardless of the selected authentication method. See [Additional Headers](#additional-headers) below |

5. **Fill Authentication Fields:** Depending on your selected authentication method (see [Authentication Methods Reference](#authentication-methods-reference) below for detailed field descriptions)
6. **Save Credential:** Click **Save** to create the credential

       ![Create Credentials](../../img/integrations/toolkits/openapi/openapi-cred-configs.png)

<Tip title="Security Recommendation">
It's highly recommended to use **[Secrets](../../menus/settings/secrets)** for API keys and sensitive values instead of entering them directly. Create a secret first, then reference it in your credential configuration.
</Tip>
     ...
     <Tab title="API Key">
          ...
          **Configuration Fields:**
          ...
          | **Custom** | Yes | For APIs with non-standard authentication headers | Custom header with specified name | **Custom Header Name** (Required): The header name where your API key will be sent. Examples: `X-API-Key`, `X-Auth-Token`, `api-key`, `X-Custom-Auth` |
            ...
            ![api-key](../../img/integrations/toolkits/openapi/auth-api-key.png)
     </Tab>

     <Tab title="OAuth (Delegated)">
          **Used for:** APIs that require delegated OAuth login with an interactive authorization flow.

          **Configuration Fields:**

          | Field | Required | Description |
          |-------|----------|-------------|
          | **Client Secret** | Yes | OAuth client secret for the delegated application |
          | **Client Id** | Yes | OAuth client identifier |
          | **Oauth Discovery Endpoint** | Yes | OAuth discovery endpoint used to initiate delegated authorization |
          | **Scope** | No | Optional OAuth scopes |

          After configuring a delegated OpenAPI credential and using it in a toolkit, the UI exposes login status and login/logout actions through the OpenAPI OAuth status components.

            <video src="../../img/integrations/toolkits/openapi/oauth-delegated.mp4" controls></video>
     </Tab>

     <Tab title="OAuth (Client Credentials)">
          **Used for:** Machine-to-machine OAuth flows where ELITEA exchanges client credentials for an access token without interactive user login.
          ...
          **Configuration Fields:**
          ...
          | **Client Id** | Yes | OAuth client identifier |
          | **Method** | No | Token exchange method. UI options are **Default (POST request)** and **Basic authorization header** |
          | **Scope** | No | Optional OAuth scopes |
          | **Token Url** | Yes | OAuth token endpoint used for the client-credentials token exchange |

          Use **Default (POST request)** unless your API documentation explicitly requires **Basic authorization header**.

          ![OAuth Client Credentials](../../img/integrations/toolkits/openapi/oauth-client-credentials.png)
     </Tab>
</Tabs>

#### Additional Headers

Independently of the authentication method selected above, you can attach one or more **static headers** to every request the toolkit sends — useful for APIs that require a secondary key alongside the primary authentication, such as an API gateway key, a tenant/organization header, or a partner identifier.

* Available in the **Additional Headers** section of the OpenAPI Configuration credential, for **any** authentication method, including Anonymous.
* Click **+ Add header**, then provide a **Header Name** and a **Header Value** for each pair.
* Header values are masked and stored with the same encryption as other credential secrets.
* Header names cannot contain spaces, `/`, `:`, or line breaks; values cannot be empty or contain line breaks.

![Additional Headers](../../img/integrations/toolkits/openapi/additional-headers.png)

<Note>
**Primary Authentication Always Wins**

If an additional header uses the same name as the header produced by your primary authentication method (for example, `Authorization`), the primary authentication value is used and the additional header is ignored. Additional headers cannot override or spoof authentication headers.
</Note>
...
<Note title="Delegated OAuth on the Toolkit Page">
If you select an OAuth (Delegated) OpenAPI credential, the toolkit configuration page displays delegated OAuth connection controls. These controls remain available after the toolkit is created.
The connection status and available actions (Login or Logout) are displayed based on the current authentication state. You can start the authentication process directly by selecting Login and completing the OAuth authentication flow.
![delegated-in-toolkit](../../img/integrations/toolkits/openapi/delegated-login-toolkit.png)
</Note>

<Accordion title="Example: Petstore API Specification">
...
#### Filtering and Limiting Large Responses

Every generated tool automatically includes three extra parameters, on top of the operation's own path/query/body fields, so agents can keep large API responses from overwhelming the conversation:

| Parameter | Description |
|-----------|--------------|
| **`response_search`** | Search a large response collection using words or quoted phrases. Bare terms contribute to BM25 relevance ranking, quoted phrases must match exactly, and terms prefixed with `-` are excluded. Example: `active "north america" -archived`. Leave empty to skip searching. |
| **`response_limit`** | Maximum number of collection items to return, from **1 to 200** (default **50**). Can be used alone, without `response_search`, to preview the first items of a large response. |
| **`regexp`** *(legacy)* | A regular expression that removes matching text from the raw response output. Cannot be combined with `response_search` or `response_limit`. |

When `response_search` and/or `response_limit` are used, the toolkit automatically locates the array or collection inside the JSON response (using the operation's response schema when available, falling back to heuristics), ranks and/or truncates its items, and returns a bounded JSON payload wrapped with an `_elitea_response_selection` metadata object describing the collection path, total item count, matched item count, and whether the result was truncated. Plain-text (non-JSON) responses are split into paragraphs or lines and filtered the same way.

<Note title="regexp Is Legacy">
`regexp` predates structured response selection and only performs raw text removal. For new use cases, prefer `response_search` and `response_limit`, which understand the response's JSON structure and return relevance-ranked, bounded results. Attempting to use `regexp` together with `response_search` or `response_limit` on the same call raises an `incompatible_response_controls` error.
</Note>
...
<Accordion title="Data Retrieval and Analysis">
...
<Accordion title="Workflow Automation and Orchestration">
...
<Accordion title="Multi-Service Integrations">
...
<Accordion title="Pattern 1: Read-Only Data Access">
...
<Accordion title="Pattern 2: CRUD Operations">
...
<Accordion title="Pattern 3: Multi-Step Workflows">
...
<Accordion title="Pattern 4: Polling and Monitoring">
...
<Accordion title="Pattern 5: Event-Driven Actions">
...
<Accordion title="Schema Parsing Errors">
...
<Accordion title="Missing Operation IDs">
...
<Accordion title="Additional Header Validation Errors">
**Problem:** "Invalid additional header name" or "Additional header '...' must have a value" when saving an OpenAPI Configuration credential

**Troubleshooting Steps:**

1. Check the **Header Name** field — it cannot be empty and cannot contain spaces, `/`, `:`, or line breaks
2. Check the **Header Value** field — it cannot be empty and cannot contain line breaks
3. Remove any duplicate header names; each name must be unique within the credential
4. Remember that an additional header sharing a name with your primary authentication header (e.g. `Authorization`) is ignored, not an error — rename it if you intended it to take effect
</Accordion>

<Accordion title="Response Filtering Errors">
**Problem:** `incompatible_response_controls` error - "'regexp' cannot be combined with 'response_search' or 'response_limit'", or `invalid_response_selection` error when calling a tool

**Troubleshooting Steps:**

1. Use either the legacy `regexp` parameter **or** `response_search`/`response_limit` on a given call, never both
2. Ensure `response_search` uses valid syntax: bare words, `"quoted phrases"`, and `-excluded` terms; a lone `-` is not valid
3. Ensure `response_limit` is an integer between **1 and 200**
4. If the error mentions an ambiguous or missing collection, the response may not contain a clear array/collection for the toolkit to filter — omit `response_search`/`response_limit` and inspect the raw response instead
</Accordion>

<Accordion title="Missing Base URL">
...
<Accordion title="Invalid JSON Body">
...
<Accordion title="HTTP Errors (4xx, 5xx)">
...
<Accordion title="Operation Not Found">
...
<Accordion title="Can I use multiple OpenAPI toolkits in the same agent?">
Yes, you can add multiple OpenAPI toolkits to a single agent, each configured with different APIs and specifications.
</Accordion>

<Accordion title="Does the toolkit support OpenAPI 2.0 (Swagger) specifications?">
Yes, both OpenAPI 2.0 (Swagger) and OpenAPI 3.x specifications are supported.
</Accordion>

<Accordion title="Can I use YAML instead of JSON for my OpenAPI specification?">
Yes, both JSON and YAML formats are fully supported. The toolkit will automatically parse either format.
</Accordion>

<Accordion title="What happens if my API specification is very large (hundreds of endpoints)?">
...
<Accordion title="Can I update my OpenAPI specification after creating the toolkit?">
Yes, edit your toolkit configuration and update the Schema field with the new specification, then save the changes.
</Accordion>

<Accordion title="How do I handle APIs that require multiple authentication methods?">
...
<Accordion title="Can I use the toolkit with internal/private APIs?">
...
<Accordion title="What if my API uses custom authentication not supported by standard methods?">
...
<Accordion title="How do I test if my OpenAPI toolkit is configured correctly?">
...
<Accordion title="Can I use the same OpenAPI Configuration credential across multiple toolkits?">
Yes, that's the recommended approach! Create one credential and reuse it across all toolkits that access the same API.
</Accordion>

<Accordion title="How do I find the OpenAPI specification for an API?">
...
<Accordion title="How do I determine which tool name to use in agent instructions?">
...
<Accordion title="What's the difference between OpenAPI Configuration and direct toolkit authentication?">
...
<Accordion title="How do I pass request bodies to POST/PUT/PATCH operations?">
...
<Accordion title="How can I add custom headers to specific API calls?">
...
<Accordion title="How do I handle APIs with pagination?">
...
<Accordion title="How do I keep a large API response from overwhelming the agent?">
Use the `response_search` and `response_limit` parameters available on every generated tool — see [Filtering and Limiting Large Responses](#filtering-and-limiting-large-responses). `response_limit` alone caps the number of items returned (1–200, default 50); `response_search` additionally ranks and filters items by relevance using words, quoted phrases, and `-excluded` terms.
</Accordion>

<Accordion title="Why aren't all my API endpoints showing up?">
...
<Accordion title="Is there a limit on the number of API endpoints I can use?">
...
For issues, questions, or assistance with OpenAPI integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.


<Info title="Useful Links">
