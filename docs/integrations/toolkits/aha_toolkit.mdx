...
| **Write** | | | |
| | **Manage record** | Legacy combined interface to create, update, or delete an Aha! record. Accepted types: `feature`, `requirement`, `idea`, `release`, `initiative`, `epic`, `page`. Parent scope is required on `create` (feature → release, requirement → feature, idea/release/initiative/page → product, epic → release). Releases and initiatives also require `parent_id` on update/delete because Aha! scopes those mutation endpoints by product. | One-tool dispatcher for the full CRUD lifecycle |
| | **Create record** | Create an Aha! record under its required parent scope. Parent scoping is release for features and epics, feature for requirements, and product for ideas, releases, initiatives, and pages. | Preferred create-only interface when you want to expose only record creation |
| | **Update record** | Update fields on an existing Aha! record. Releases and initiatives require `parent_id` because Aha! scopes those mutation endpoints by product. | Preferred update-only interface when you want to expose only record updates |
| | **Delete record** | Delete an existing Aha! record. Releases and initiatives require `parent_id` because Aha! scopes those mutation endpoints by product. | Preferred delete-only interface when you want to expose only record deletion |
...
| **Comments** | | | |
| | **Add comment** | Post a comment on an Aha! record. Supports features, requirements, ideas, releases, release phases, epics, initiatives, goals, pages, and to-dos. Body accepts HTML or plain text. | Automate progress updates and cross-team notifications |
| | **List comments** | List comments on an Aha! record (paginated) | Access discussion history |
| **Attachments** | | | |
| | **Attach file** | Upload an attachment to an Aha! record description or to-do. The toolkit resolves the record's description note ID when needed and uploads the file through Aha!'s attachments API. | Attach evidence, screenshots, or generated documents to a record |
| **Metadata** | | | |
