---
title: "Figma"
description: "Connect ELITEA to Figma — access design files, components, and comments to automate design-related workflows."
---
...
3.  **Follow Registration Process:** Follow the on-screen instructions to complete the registration process and create your Figma account. You can sign up using your email address or connect with your Google account.
    ...
    <img src="../../img/integrations/toolkits/figma/Figma-SignUpProcess.png" alt="Figma-SignUpProcess" />
...
1.  **Access Figma Settings:** In Figma, click on your profile icon (typically your avatar or initials) located in the top left corner of the Figma interface and navigate to **"Settings"**. **Note:** Ensure you have the correct Figma account selected if you are logged into multiple Figma accounts.
    ...
    <img src="../../img/integrations/toolkits/figma/Figma-SettingsMenu.png" alt="Figma-SettingsMenu" />
...
3.  **Generate New Personal Access Token:** Scroll down to the "Personal access tokens" section and click on the **"Generate new token"** button.
    ...
    <img src="../../img/integrations/toolkits/figma/Figma-SecurityTab.png" alt="Figma-SecurityTab" />
...
1. **Navigate to Credentials Menu:** Open the sidebar and select [Credentials](../../how-tos/credentials-toolkits/how-to-use-credentials#figma-credential-setup).
...
6. **Save Credential:** Once the test is successful, click **Save** to create the credential.
      ...
      <img src="../../img/integrations/toolkits/figma/figma-cred-create.gif" alt="Figma Credential" />

<Tip title="Security Recommendation">
It's highly recommended to use **[Secrets](../../menus/settings/secrets)** for tokens instead of entering them directly. Create a secret first, then reference it in your credential configuration.
</Tip>
...
8. **Save Toolkit:** Click **Save** to create the toolkit.
     ...
     <img src="../../img/integrations/toolkits/figma/figma-toolkit-config.gif" alt="Figma Credential" />

<Info title="Alternative: Create from Canvas">
You can also create a Figma Toolkit directly from a chat conversation using the canvas interface. This allows immediate use without navigating away from your conversation. For details, see [How to Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas).
</Info>
...
| | **Extract design tokens** | Extracts design tokens from a single Figma node and returns a structured summary of its core visual values like colors, typography, and effects | Extract design tokens for documentation, implementation handoff, or token auditing |
| | **Extract design tokens batch** | Extracts design tokens from multiple Figma nodes in one request | Efficiently manage and update design tokens across projects |
...
<img src="../../img/integrations/toolkits/figma/figma-add-agent.gif" alt="Figma-Agent" />

Your agent can now interact with Figma using the configured toolkit and enabled tools.

<Info title="Reference">
For more information on agent configuration and management, see the [Agent Guide](../../menus/agents).
</Info>
...
<img src="../../img/integrations/toolkits/figma/figma-add-pipeline.gif" alt="Figma-Pipeline" />

#### **In Chat:**

1. **Navigate to Chat:** Open the sidebar and select **[Chat](../../menus/chat)** or use the main chat interface.
...
<img src="../../img/integrations/toolkits/figma/figma-add-chat.gif" alt="Figma-chat" />


<Info title="Reference">
For more information on chat functionality, see the [Chat Guide](../../menus/chat).
</Info>
...
1. **From Project URL (If You Are Project Admin) - Web & Desktop:** When viewing a project in Figma (web or desktop app), the Project ID is in the URL:
    ...
    **Desktop App:** Use **File → Copy link** or **Share** to get the project URL with the same format.
     ...
     <img src="../../img/integrations/toolkits/figma/figma-projectid.png" alt="Figma-url" />
...
<Info title="Project ID Usage Note">
...
4. **From Recent Files:** Right-click on a file in your recent files list and select **Copy link**
      ...
      <img src="../../img/integrations/toolkits/figma/Figma-url.png" alt="Figma-url" />
...
**Node ID Format:** Node IDs typically appear as `123:456` in the URL parameter `node-id=123%3A456` (where `%3A` is the URL-encoded colon).
  ...
  <img src="../../img/integrations/toolkits/figma/figma-node-id.png" alt="Figma-url" />
...
3. **From Account Settings:** Access your account settings in the desktop app and navigate to team management where Team IDs may be displayed
 ...
 <img src="../../img/integrations/toolkits/figma/figma-team-id.png" alt="Figma-url" />
...
<Accordion title="1. Quick Design Analysis in Chat">
...
<img src="../../img/integrations/toolkits/figma/analyze-file.gif" alt="Figma Analysis" width="600" />
...
<Accordion title="2. Automated Design Review and Documentation in Chat">
...
<Accordion title="3. Design System Audit in Chat">
...
<Accordion title="4. UX Research and User Flow Analysis in Chat">
...
<Accordion title="5. Accessibility Compliance Check in Chat">
...
<Accordion title="6. Version Comparison in Chat">
...
<Accordion title="7. Design Search with Indexing in Chat">
...
<Accordion title="8. Automated Design Review Agent (Agent Configuration Example)">
...
<Info title="Agent Instructions Guidelines">
...
<Accordion title="No File or Nodes Found Using Correct Request IDs">
...
<Accordion title="Receiving Partial JSON or Incomplete Answers">
...
<Accordion title="Values Cut by Global Regex Filter">
...
<Accordion title="Toolkit Connection Fails">
...
<Accordion title="Toolkit Fails to Post Comments">
...
<Accordion title="Can I use multiple Figma toolkits in the same agent?">
Yes, you can add multiple Figma toolkits to a single agent, each configured with different files or credentials.
</Accordion>

<Accordion title="Does the toolkit support Figma Community files?">
...
<Accordion title="Can I use YAML for the Global Regexp filter?">
...
<Accordion title="What happens if my Figma file is very large (hundreds of frames)?">
...
<Accordion title="Can I update my Figma credentials after creating the toolkit?">
...
<Accordion title="How do I handle multiple Figma teams or organizations?">
...
<Accordion title="Can I use the toolkit with Figma files that require OAuth 2.0?">
Currently, the toolkit supports Personal Access Token authentication. OAuth 2.0 support may be added in future releases.
</Accordion>

<Accordion title="What if my Figma file uses custom plugins or extensions?">
...
<Accordion title="How do I test if my Figma toolkit is configured correctly?">
...
<Accordion title="Can I use the same Figma credential across multiple toolkits?">
...
<Accordion title="How do I find my Figma file key?">
...
<Accordion title="Can I restrict which Figma tools are available to an agent?">
...
If you encounter issues not covered in this guide or need additional assistance with Figma integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.

---

<Info title="Useful ELITEA Resources">
To learn more about using the Figma toolkit with ELITEA, explore these related guides and references:

  * **[ELITEA Credentials Management](../../how-tos/credentials-toolkits/how-to-use-credentials)** - *Learn how to securely store your Figma API Token using ELITEA's Credentials management feature for enhanced security within ELITEA.*
  ...
  * **[Index Figma Data](../../how-tos/indexing/index-figma-data)** - *Detailed instructions for indexing Figma design files to enable advanced search, analysis, and AI-powered insights across your design assets.*
</Info>

<Info title="External Resources">
*   **Figma Website:** [https://www.figma.com/](https://www.figma.com/) - *Access the main Figma product website for product information, documentation, and tutorials.*
