---
title: "SharePoint"
description: "Connect ELITEA to SharePoint — read, write, and index documents and lists from SharePoint sites and document libraries."
---
...
<Info title="URL is not shown in the UI">
...
<Info title="Delegated Permissions">
...
<Warning title="Azure ACS retirement for SharePoint Online">
Microsoft has retired **Azure ACS (Access Control Services)** for SharePoint Online and fully disabled it for existing tenants as of **02-Apr-2026**. Microsoft recommends moving SharePoint Online integrations to **Microsoft Entra ID**-based authentication.

For ELITEA SharePoint toolkit usage in SharePoint Online:

- Prefer **Delegated (User OAuth)** credentials for ongoing supported access.
- Treat legacy **App-Only** patterns that depend on Azure ACS as migration scenarios, not as the recommended long-term setup.
- If you are assessing older SharePoint Online integrations, review Microsoft's Azure ACS retirement guidance and migration recommendations before continuing with production use.

This retirement does **not** apply to SharePoint Server on-premises hybrid scenarios in the same way, but it does apply to SharePoint Online and Project Online.

For details, see Microsoft's **[Azure ACS retirement announcement](https://learn.microsoft.com/ru-ru/sharepoint/dev/sp-add-ins/retirement-announcement-for-azure-acs)**.
</Warning>
...
The SharePoint credential supports two authentication modes — **App-Only** (the application acts as itself) and **Delegated** (the application acts on behalf of a signed-in user). For **SharePoint Online**, **Delegated** is the recommended option because Microsoft has retired Azure ACS and recommends moving to Microsoft Entra ID-based authentication. Select the tab in the ELITEA credential form that matches your organization's access policy:
...
<Warning title="App-Only for SharePoint Online">
Use this option with caution for **SharePoint Online**. If your existing App-Only setup depends on **Azure ACS**, it is no longer a viable long-term authentication model after Microsoft's Azure ACS retirement. For new or migrated SharePoint Online setups, prefer **Delegated (User OAuth)** unless you have confirmed a supported Microsoft Entra ID-based application pattern for your scenario.
</Warning>
...
2. **Log in:** Once the **OAuth Discovery Endpoint** field is filled in, a **Login** button appears next to the **Test Connection** button. Click **Login** to initiate the OAuth authorization flow — ELITEA calls the SharePoint check-connection endpoint, which redirects to Azure AD for user sign-in. After you authorize the application, the token is stored and the connection is verified via Microsoft Graph.

    > After a successful login, the **Login** button changes to **Logout**. Click **Logout** at any time to clear the stored token and disconnect the credential.
...
Use **[Secrets](../../menus/settings/secrets)** for sensitive values (Client Secret) instead of entering them directly. Create a secret first, then reference it in your credential configuration.
</Tip>
...
> For **SharePoint Online**, review this option carefully against Microsoft's Azure ACS retirement guidance. In most ongoing SharePoint Online scenarios, **Delegated** is the safer recommended choice.
...
- You are configuring **SharePoint Online** access and want the authentication model aligned with Microsoft's post-Azure-ACS guidance.
...
5. **Enable Desired Tools:** In the **"Tools"** section, select the checkboxes next to the specific SharePoint tools you want to enable. **Enable only the tools your agents will actually use** to follow the principle of least privilege.
       * **[Make Tools Available by MCP](../mcp/make-tools-available-by-mcp)** — (optional checkbox) Enable this option to make the selected tools accessible through external MCP clients.
...
#### OAuth Status Bar (Delegated mode only)

When the selected SharePoint credential uses **Delegated (User OAuth)** authentication, a connection status bar appears at the top of the toolkit configuration form. This bar is hidden for App-Only credentials.

The status bar displays:
- A colored status indicator with **"Connected!"** (green) when a valid OAuth token is present, or **"Not Connected"** (orange/warning) when no token exists or the token has expired.
- A **Login** button when not connected, or a **Logout** button when connected.

You can use this status bar to authorize (or re-authorize) the Delegated connection directly from the toolkit form — without navigating back to the Credentials page. Click **Login** to open the OAuth authorization flow, or **Logout** to clear the stored token.

     ![Toolkit](../../img/integrations/toolkits/sharepoint/sharepoint-toolkit-status-bar.gif)

<Info title="Token scope and re-login">
The Login button in the toolkit form uses the scopes configured in the linked SharePoint credential. If the scopes were changed on the credential after the last login, click **Logout** then **Login** again to obtain a fresh token with the updated scopes.
</Info>
...
* The SharePoint toolkit provides the following tools for interacting with SharePoint sites, document libraries, and lists, organized by functional categories:
    ...
    |:-----------------:|---------------|-----------------|----------------------|
    | **File Operations** | | | |
    | | **Get files list** | Lists files in a SharePoint document library, optionally filtered by folder or library name; supports `include_extensions` and `skip_extensions` filters to narrow results by file type | Browse and discover files across document libraries |
    | | **Read document** | Reads the content of a document at a given server-relative path; supports `.md`, `.docx`, `.pptx`, `.xlsx`, `.py`, `.java`, `.txt`, `.json`, `.js`, and `.pdf`; optional `page_number` (for multi-page documents), `sheet_name` (Excel), and `is_capture_image` (AI image description) parameters | Extract text content from SharePoint documents for analysis or summarization |
    | | **Read file from sharing link** | Reads the content of a file from a public or sharing link | Access files shared via a link without needing to import them into a workspace first |
    ...
    | | **Onenote read page** | Reads and converts a OneNote page into beautified plain text; supports `capture_images` (AI image descriptions, default on), `include_attachments` (list attachments inline, default on), and `read_attachment_content` (parse attachment content inline, default off) | Retrieve human-readable page content for summarization or analysis |
    | | **Onenote read page items** | Reads a OneNote page into a structured list of typed items (text, image, attachment) in document order; supports `capture_images`, `include_attachments`, and `read_attachment_content` parameters | Process page content programmatically with fine-grained control over each element |
    ...
    | **Indexing & Search** | | | |
    | | **Index data** | Creates searchable indexes of SharePoint document library content; supports `include_onenote` flag (with optional `onenote_filter`) to also index OneNote pages and attachments alongside document library files | Enable advanced semantic search across SharePoint files and OneNote pages |
...
3. **Add SharePoint Toolkit:**
     ...
     * If the toolkit uses a **Delegated (User OAuth)** credential, a **connection status icon** (globe indicator) and a **Log in** button appear at the right side of the toolkit row. Click **Log in** to authorize the SharePoint connection directly from the agent configuration — no need to leave the page. Once authorized, the icon switches from offline (orange) to online (green).
...
<Info title="Delegated credential: Login button in the toolkit card">
The same connection indicator described for Agents also appears in Pipeline toolkit cards for SharePoint toolkits with Delegated credentials — an offline icon with a **Log in** button when not authorized, and an online icon when connected.
</Info>
...
3. **Add Toolkit to Conversation:**
     ...
     * If the toolkit uses a **Delegated (User OAuth)** credential, a **connection status icon** (globe indicator) appears next to the toolkit name in the Participants list — **green** when authorized, **orange** when not. If not authorized, a **Log in** link appears below the toolkit entry; click it to complete the OAuth flow without leaving the Chat.
...
<Info title="Delegated credential: authorization prompt in Chat">
If the SharePoint toolkit uses a **Delegated (User OAuth)** credential and no valid token is present, a warning appears below the toolkit entry in the Participants panel:

> *SharePoint requires authorization. **Log in.***

Click the **Log in** link to complete the OAuth authorization flow without leaving the Chat. Once authorized, the connection icon next to the toolkit name in the Participants list changes from offline (orange) to online (green) and the warning disappears.
</Info>
...
<Info title="Supported spaces">
SharePoint toolkit supports different types of SharePoint sites (including classic (/sites/) and team (/team/) sites and private SharePoint spaces (my.sharepoint.com)).
</Info>
...
<Accordion title="File Operations - Get All Files">
**Chat Example:**
```
...
<Accordion title="File Operations - Read Document">
**Chat Example:**
```
...
<Accordion title="File Operations - Upload File">
**Chat Example:**
```
...
<Accordion title="List & Data Operations - Get Lists">
**Chat Example:**
```
...
<Accordion title="List & Data Operations - Read List">
**Chat Example:**
```
...
<Accordion title="List & Data Operations - Create List Item">
**Chat Example:**
```
...
<Accordion title="Indexing & Search - Search Index">
**Chat Example:**
```
...
<Accordion title="Incorrect Library or List Name">
...
<Accordion title="File Path Errors When Reading Documents">
...
<Accordion title="Create List Item Fails with Field Errors">
...
<Accordion title="OneNote Tools Return 'Graph API Delegated Access Required' Error">
...
<Accordion title="OneNote Tools Return 401 Unauthorized Error">
...
<Info title="Required OneNote Scopes">
...
<Info title="Short vs. Full URL scope format">
...
If you encounter issues not covered in this guide or need additional assistance with SharePoint integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.

---

## FAQ

<Accordion title="Can I use my regular SharePoint username and password instead of an Azure AD App Registration?">
...
<Accordion title="What are the required credential fields for the SharePoint toolkit?">
...
<Accordion title="Do I need to complete the AppInv.aspx step even if I already granted Microsoft Graph permissions in Azure AD?">
...
<Accordion title="Do I need to register a Redirect URI for the Delegated auth flow?">
...
<Accordion title="What is the correct format for the Site URL in the SharePoint credential?">
...
<Accordion title="Which tools require PgVector and an Embedding Model?">
...
<Accordion title="Can I use the same SharePoint credential across multiple toolkits?">
...
<Accordion title="Why is `create_list_item` failing with a validation error?">
...
<Accordion title="Why am I consistently encountering 'Permission Denied' errors even though I believe I configured everything correctly?">
...
To learn more about using the SharePoint toolkit with ELITEA, explore these related guides and references:

  * **[How to Use Chat Functionality](../../how-tos/chat-conversations/how-to-use-chat-functionality)** — *Complete guide to using ELITEA Chat with toolkits for interactive SharePoint operations.*
  ...
  * **[Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas)** — *Discover how to create and configure SharePoint toolkits directly from the chat interface.*
  * **[Create and Edit Pipelines from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-pipelines-from-canvas)** — *Guide to building and modifying pipelines from chat canvas for automated SharePoint workflows.*
  * **[Indexing Overview](../../how-tos/indexing/indexing-overview)** — *Comprehensive guide to understanding ELITEA's indexing capabilities for enhanced search and discovery.*
  * **[Index SharePoint Data](../../how-tos/indexing/index-sharepoint-data)** — *Detailed instructions for indexing SharePoint document library data to enable advanced semantic search.*
</Info>
