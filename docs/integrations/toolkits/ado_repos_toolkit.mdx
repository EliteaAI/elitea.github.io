---
title: "ADO Repos"
description: "Connect ELITEA to Azure Repos — manage repositories, branches, commits, pull requests, and code reviews from your agents."
---
...
6.  **Copy and Store Token:** **Copy the generated token immediately** - this is your only chance to see it. Store it securely in a password manager or ELITEA's **[Secrets](../../menus/settings/secrets)** feature.

    <img src="../../img/integrations/toolkits/ado/ado-new-token.gif" alt="AdoRepos-Generate_PAT" />
...
6. **Save Credential:** Click **Save** to create the credential. It will be available in the Credentials dashboard for use in toolkit configurations.
     ...
     <img src="../../img/integrations/toolkits/ado/ado-credentials-create.gif" alt="Credentials" />

<Tip title="Security Recommendation">
Use **[Secrets](../../menus/settings/secrets)** for your PAT instead of entering values directly. Create a secret first, then reference it in your credential configuration.
</Tip>
...
3. **Select Azure Repos:** Choose **Azure Repos (ADO Repos)** from the list of available toolkit types.
    ...
    <img src="../../img/integrations/toolkits/adorepos/ado-repo-access.png" alt="Azure Repos Toolkit Access" />
...
4. **Configure Toolkit Settings:**
    ...
    | **ADO configuration** | Select your previously created Azure DevOps credential from the dropdown. A credential may be pre-selected — verify it is correct or change it as needed. | `Azure DevOps - Team Project Access` |
    ...
    | **Repositories** | One or more repository IDs or names in the project, separated by commas. Optional — leave empty to let tools target any repository in the project | `my-service, my-service-tests` |
...
5. **Enable Desired Tools:** In the **"Tools"** section, select the checkboxes next to the specific Azure Repos tools you want to enable. **Enable only the tools your agents will actually use** to follow the principle of least privilege
       * **[Make Tools Available by MCP](../mcp/make-tools-available-by-mcp)** - (optional checkbox) Enable this option to make the selected tools accessible through the external MCP clients to use the toolkit's capabilities
6. **Save Toolkit:** Click **Save** to create the toolkit.

<video controls>
  <source src="../../img/integrations/toolkits/adorepos/ado-repo-toolkit-create.mp4" type="video/mp4"/>
</video>
 

<Note>
**Configuring One or More Repositories**

Most tools (all except **List repositories** and the vector-search tools) take an optional or required per-call **`repository_id`** (name or ID), depending on how many repositories are configured:

| Repositories configured | `repository_id` | Behavior |
|---|---|---|
| **One** (default) | Optional | Defaults to that repository. |
| **Several** | Optional, restricted to the list | Defaults to the *first* one; others must be named explicitly. |
| **None** | **Required** | Any repository in the project can be targeted. |

**Branch resolution:** validated at save time for one repository; deferred and cached per repository, with automatic fallback, for several or none. Legacy toolkits saved with a single repository string keep working unchanged.

Use the **List repositories** tool at any time to discover the repositories a toolkit can access, along with their IDs and default branches — useful when no repository is configured, or when you prefer IDs over names in the **Repositories** field.
</Note>
...
|:------------:|---------------|-----------------|
| **Read** | | |
| | **Get commits** | Retrieves commit history and details |
| | **Get pull request** | Retrieves details of a specific pull request |
| | **Get work items** | Retrieves work items linked to pull requests |
| | **List branches in repo** | Retrieves all branches in the repository |
| | **List files** | Lists files in the active branch |
| | **List indexes** | Lists available indexed indexes |
| | **List open pull requests** | Retrieves list of open pull requests |
| | **List pull request files** | Lists files changed in a pull request |
| | **List repositories** | Lists the repositories this toolkit can access, with each one's default branch |
| | **Read file** | Reads content of a specific file |
| | **Search code** | Search code across the repository using keywords |
| | **Search index** | Searches indexed repository data |
| | **Stepback search index** | Performs advanced stepback search on indexed data |
| | **Stepback summary index** | Generates summaries from stepback search results |
| **Create & update** | | |
| | **Comment on pull request** | Adds comments to a pull request |
...
| | **Index data** | Indexes repository data for semantic search |
| | **Set active branch** | Sets a specific branch as active for operations |
| | **Update file** | Updates content of an existing file |
| **Delete** | | |
| | **Delete file** | Deletes a file from the repository |
| | **Remove index** | Removes indexed data from the system |
...
<Note>
**Search Code Tool - Branch Indexing**

The **Search code** tool uses Azure DevOps Code Search, which only indexes the repository's default branch by default. 

**To search code in non-default branches:**
1. Navigate to **Project Settings** → **Repositories** → **Options** → **Searchable branches** in your Azure DevOps project
2. Add the specific branches you want to search
3. Wait for Azure DevOps to index those branches

**Important notes:**
- Forked repositories are never indexed by Azure DevOps Code Search
- This indexing is handled on the Azure DevOps side and is separate from ELITEA's **Index data** tool functionality

<img src="../../img/integrations/toolkits/adorepos/ado-searchable-branches.png" alt="Configure Searchable Branches" />
</Note>
...
3. **Add Azure Repos Toolkit:** 
    ...
    <img src="../../img/integrations/toolkits/adorepos/ado-repo-agent-add.png" alt="Agents" />     
...
3. **Add Azure Repos Toolkit:** 
     ...
     * The toolkit will be added to your pipeline with the previously configured tools enabled
     ...
     <img src="../../img/integrations/toolkits/adorepos/ado-repo-pipeline-add.png" alt="Agents" /> 
...
3. **Add Ado Repos Toolkit to Conversation:**
   - Click the **`+`** icon in the chat input toolbar (tooltip: *Add files, agents, toolkits and more…*)
   - Hover over **Toolkits** in the popup menu
   - Search for your Ado Repos toolkit by name
   - Enable the toolkit to add it to your conversation
4. **Verify Toolkit Added:** The Ado Repos toolkit appears in the **PARTICIPANTS** panel on the right side under the **Toolkits** section.
5. **Use Toolkit in Chat:** You can now directly interact with your Azure DevOps Repos by asking questions or requesting actions that will trigger the Ado Repos toolkit tools.

<Tip>
You can also add the toolkit by typing **`#`** in the chat input box and searching for your Ado Repos toolkit.
</Tip>

     <img src="../../img/integrations/toolkits/adorepos/ado-repo-chat-add.png" alt="Ado Repos Toolkit" /> 
...
<Info>
**Targeting a Repository When Several Are Configured**

If the toolkit has more than one repository configured (or none), tell the Agent which repository to use by including `repository_id` in your tool instructions. For example: *"Tool: Use the 'read_file' tool. Parameters: repository_id: 'backend-api', file_path: 'README.md'."*
</Info>
...
<Accordion title="Branch Management - Create Branch">
**User Request:**
```
...
<Accordion title="File Operations - Read File">
**User Request:**
```
Show me the content of the README.md file.
```
...
<Accordion title="File Operations - Update File">
**User Request:**
```
...
<Accordion title="Pull Request Management - Create Pull Request">
**User Request:**
```
...
<Accordion title="Pull Request Management - List Open Pull Requests">
**User Request:**
```
...
<Accordion title="Work Item Integration - Get Work Items">
**User Request:**
```
...
<Accordion title="Repository Discovery - List Repositories">
**User Request:**
```
What repositories can this toolkit access?
```

**Agent Actions:**
1. Uses `list_repositories` tool

**Agent Response:**
```
Here are the repositories this toolkit can access:

1. frontend-app (default branch: main)
2. backend-api (default branch: develop)
```
</Accordion>

<Accordion title="Multi-Repository - Targeting a Specific Repository">
**User Request:**
```
Show me the content of the README.md file in the 'backend-api' repository.
```
...
**Agent Response:**
```
Here's the content of README.md from backend-api:
...
<Accordion title="Incorrect Repository or Project Names">
...
2. **Check Repository Value:** Ensure that the repository name or ID you are using exactly matches an entry in the toolkit's **Repositories** field (or, if that field is empty, an actual repository in the project). Use the **List repositories** tool to confirm valid values.
...
<Accordion title="Repository Parameter Errors in Multi-Repository Toolkits">
**Problem:** Agent execution fails with an error such as *"Parameter 'repository_id' is required: this toolkit is not configured with any repository"* or *"Repository 'X' is not available in this toolkit. Configured repositories: ..."*

**Cause:**
- The toolkit's **Repositories** field is empty, so `repository_id` must be supplied on every tool call — but the Agent (or your instructions) didn't provide one.
- The toolkit's **Repositories** field lists specific repositories, and the Agent tried to target one that isn't in that list.

**Resolution:**
1. If the toolkit has no configured repositories, instruct the Agent to call **List repositories** first to discover valid IDs/names, then pass one explicitly as `repository_id` on subsequent tool calls.
2. If the toolkit has configured repositories, verify the `repository_id` value matches one of them exactly (both names and IDs are accepted, but the value must match a configured entry).
3. To make an additional repository available, edit the toolkit and add it to the **Repositories** field, separated by a comma.
</Accordion>

<Accordion title="Repository ID Retrieval Issues">
**Problem:** Unable to retrieve the Repository ID using the provided curl command or other methods.

**Troubleshooting Steps:**

1. **Verify curl Command Format:** Double-check that you've correctly replaced `{PAT}`, `{organization}`, and `{project}` placeholders, and that the PAT is formatted correctly: `curl -u {PAT}:` (note the colon after PAT).
2. **Use the Repository Name Instead:** The **Repositories** field also accepts repository names directly, so an ID is not strictly required.
</Accordion>
...
<Accordion title="Branch Not Found Error at Toolkit Creation">
**Problem:** Toolkit creation fails with an error such as *"The base branch 'main' does not exist"* or *"The active branch 'feature-xyz' does not exist."*

**Cause:** The branch names entered in the **Base branch** or **Active branch** fields of the toolkit configuration do not match any existing branch in the repository. Branch names in Git are case-sensitive.

**Resolution:**
1. Open your repository in Azure DevOps and verify the exact default branch name — it may be `main`, `master`, `develop`, or a custom name
2. Correct the branch name(s) in the toolkit settings — ensure capitalization matches exactly
3. If the branch was recently deleted or renamed, update the toolkit configuration accordingly
4. After saving the toolkit, use the **List branches in repo** tool to confirm available branches
</Accordion>

<Accordion title="Search Code Returns No Results or Cannot Search Non-Default Branch">
**Problem:** The **Search code** tool returns no results when searching in a non-default branch, or searches only return results from the default branch.

**Cause:** Azure DevOps Code Search only indexes the repository's default branch by default. Non-default branches must be explicitly configured as searchable branches in Azure DevOps project settings. Additionally, forked repositories are never indexed by Azure DevOps Code Search.

**Resolution:**

1. **Add Branch to Searchable Branches:**
   - Navigate to **Project Settings** → **Repositories** → **Options** → **Searchable branches** in your Azure DevOps project
   - Click **Add branch** and select the branch you want to search
   - Wait for Azure DevOps to index the newly added branch (this may take a few minutes depending on repository size)

2. **Verify Default Branch:** If you're trying to search the default branch and still getting no results, verify that Code Search is enabled for your Azure DevOps organization

3. **Forked Repository Limitation:** If you're working with a forked repository, note that Azure DevOps Code Search does not index forked repositories. You must search the original repository instead.

4. **Indexing vs ELITEA Indexing:** Remember that this Azure DevOps indexing is separate from ELITEA's **Index data** tool functionality. The **Search code** tool uses Azure DevOps' native Code Search, while **Search index** uses ELITEA's own indexing.

<img src="../../img/integrations/toolkits/adorepos/ado-searchable-branches.png" alt="Configure Searchable Branches in Azure DevOps" />
</Accordion>

### Support Contact

If you encounter issues not covered in this guide or need additional assistance with Azure Repos integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.    

---

## FAQ

<Accordion title="Can I use my regular Azure DevOps password directly for the ELITEA integration instead of a Personal Access Token?">
...
<Accordion title="What scopes/permissions are absolutely necessary for the Azure DevOps Personal Access Token to work with ELITEA?">
...
<Accordion title="What format does the Repositories field expect, and can I enter more than one?">
The **Repositories** field accepts either the repository **name** or its **ID**, and you can enter **one, several (comma-separated), or none**.

**Example (single):** `my-service`

**Example (multiple):** `my-service, my-service-tests`


</Accordion>

<Accordion title="Can a single Azure Repos toolkit work with more than one repository?">
**Yes.** Enter multiple repository names or IDs, separated by commas, in the **Repositories** field.
</Accordion>
...
<Accordion title="Can I use the same Azure DevOps credential across multiple toolkits and agents?">
...
<Accordion title="Can I use Azure Repos toolkits with private repositories?">
...
<Accordion title="Can I use the same toolkit across different workspaces (Private vs. Team Projects)?">
...
<Accordion title="Why am I getting 'Repository not found' errors even though the repository exists?">
...
<Accordion title="How do I handle PAT expiration and rotation?">
...
<Accordion title="Can I read only specific lines of a file instead of the entire file?">
Yes. The `read_file` tool supports partial file reading using two optional parameters:

- **`offset`** — the 1-indexed starting line number (e.g., `offset=50` starts reading from line 50)
- **`limit`** — the maximum number of lines to return from the offset (e.g., `limit=30` returns 30 lines)

**Example agent instruction:**
```
Read lines 50 to 80 of the file 'src/utils/helpers.py' on the main branch.
```

If `offset` is omitted, reading starts from the beginning. If `limit` is omitted, all lines from the offset to the end of the file are returned.
</Accordion>

<Accordion title="Can I filter commit history by date, author, or file path?">
Yes. The `get_commits` tool supports the following optional filters:

| Parameter | Description | Example |
|-----------|-------------|--------|
| `sha` | Start from a specific commit SHA | `abc123def456` |
| `path` | Only commits that modified this file or folder | `src/api/routes.py` |
| `since` | Only commits after this date (ISO 8601) | `2025-01-01T00:00:00Z` |
| `until` | Only commits before this date (ISO 8601) | `2025-12-31T23:59:59Z` |
| `author` | Filter by author username | `john.smith` |

**Example agent instruction:**
```
Get all commits made to 'src/auth/login.py' by 'jane.doe' since 2025-06-01.
```
</Accordion>

<Accordion title="Can the agent add inline code comments to specific lines in a pull request?">
Yes. The `comment_on_pull_request` tool supports inline comments targeting specific file lines or line ranges using the `inline_comments` parameter:
...
[
  ...
  },
  {
    ...
    "comment_text": "Consider extracting this into a helper function.",
    "right_range": [35, 37]
  }
]
```

Use `right_line` to target a single line, or `right_range` to target a range of lines. Use `left_line`/`left_range` for comments on the old (base) version of the file.
</Accordion>

---

<Info title="Useful ELITEA Resources">
To learn more about using the Azure Repos toolkit with ELITEA, explore these related guides and references:

* **[How to Use Chat Functionality](../../how-tos/chat-conversations/how-to-use-chat-functionality)** - *Complete guide to using ELITEA Chat with toolkits for interactive Azure DevOps operations.*
...
* **[Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas)** - *Discover how to create and configure Azure Repos toolkits directly from chat interface for streamlined workflow setup.*
* **[Create and Edit Pipelines from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-pipelines-from-canvas)** - *Guide to building and modifying pipelines from chat canvas for automated Azure DevOps workflows.*
...
* **[Index Repo Data](../../how-tos/indexing/index-github-data)** - *Detailed instructions for indexing repository data to enable advanced search, analysis, and AI-powered insights across your codebase.*
</Info>
