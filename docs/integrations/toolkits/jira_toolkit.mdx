---
title: "Jira"
description: "Connect ELITEA to Jira — create, update, and query issues, sprints, and projects to automate development workflows."
---
...
3. **Select Jira:** Choose **Jira** as the credential type.
     ...
     ![Jira Credential Access](../../img/integrations/toolkits/jira/jira-credential-access.gif)

4. **Configure Credential Details:**

   | Field | Required | Description |
   |-------|----------|-------------|
   | **Display Name** | Required | Descriptive name for this credential (e.g., *"Jira - Project Management"*) |
   | **ID** | Required | Unique identifier for the credential. Auto-populated from the Display Name|
   | **Hosting** | Optional | Hosting type of your Jira instance. **Auto** (default) — detects from the Base URL automatically; **Cloud** — for Atlassian-hosted instances (`*.atlassian.net`); **Server** — for self-hosted or Data Center deployments |
   | **Base URL** | Required | Base URL of your Jira instance without the `/jira` suffix. Cloud example: `https://yourcompany.atlassian.net` · Server example: `https://jira.yourcompany.com` |
   | **Username** | Basic auth | Your Jira account email (Cloud) or login username (Server). Not required when using Bearer authentication |
   | **API Key** | Basic auth | API Token for Basic authentication. Generate at [Atlassian security settings](https://id.atlassian.com/manage-profile/security/api-tokens) (Cloud) or from Jira profile → Personal Access Tokens (Server/DC) |
   | **Token** | Bearer auth | Personal Access Token (PAT) for Bearer authentication. Generate in Jira: Profile → Personal Access Tokens |
...
3. **Select Jira:** Choose **Jira** from the list of available toolkit types.
     ...
     ![Jira create Toolkit](../../img/integrations/toolkits/jira/jira-toolkit-access.png)

4. **Configure Toolkit Details:**

   | Field | Required | Default | Description |
   |-------|----------|---------|-------------|
   | **Toolkit Name** | Required | — | Descriptive name for the toolkit (e.g., *"Jira Project Management"*) |
   | **Description** | Optional | — | Brief description of the toolkit's purpose |
   | **Jira Configuration ** | Required | — | Select the Jira credential created in Step 1 |
   ...
   | **API Version** | Optional | Auto | REST API version used for Jira operations. **Auto** (recommended) automatically selects V3 for Cloud or V2 for Server based on the credential's Hosting setting. Manually override only if Auto does not resolve correctly. See the note below for details |
   | **Limit** | Optional | 5 | Maximum number of issues returned per request. Increase for large JQL result sets (e.g., 50, 100) |
   | **Labels** | Optional | — | Comma-separated labels applied to created or updated Jira entities (e.g., `alita,elitea`) |
   | ***Advanced Settings*** | | | |
   | **Verify SSL** | Optional | Enabled | Validates the server's SSL certificate. Disable only for self-signed certificate environments |
   | **Additional Fields** | Optional | — | Custom Jira field IDs accessible within the toolkit, comma-separated (e.g., `customfield_10045,customfield_10100`) |
   | **Custom Headers** | Optional | — | Additional HTTP headers included with every API request, in JSON format (e.g., `{"X-Custom-Header": "value"}`) |

<Info title="API Version Selection">
The **API Version** field determines which Jira REST API endpoints are used:

- **Auto** (default) — Automatically selects the API version based on the **Hosting** setting in the linked credential:
  - **Cloud hosting** (base URL contains `.atlassian.net`) → **V3** (`/rest/api/3`)
  - **Server hosting** (on-premises or Data Center) → **V2** (`/rest/api/2`)

- **V3** — Jira Cloud REST API v3. Uses Atlassian Document Format (ADF) for comments and descriptions. **Cloud-only** — selecting V3 against a Server/Data Center instance will fail.

- **V2** — Jira REST API v2. **Standard for Jira Server / Data Center**; also still supported on Cloud for backward compatibility. Uses plain text and wiki markup.

⚠️ **Important:** Using the wrong version for your instance type may cause failures in comments, search, and attachments.
</Info>

5. **Enable Desired Tools:** In the **"Tools"** section, select the checkboxes next to the specific Jira tools you want to enable. **Enable only the tools your agents will actually use** to follow the principle of least privilege.
...
<video src="../../img/integrations/toolkits/jira/jira-toolkit-create.mp4" controls></video>
...
|:------------:|---------------|-----------------|
| **Read** | | |
| | **Search using JQL** | Search for Jira issues using Jira Query Language (JQL) queries |
| | **List projects** | Retrieve a list of all projects available in the Jira instance |
| | **List comments** | Retrieve comments associated with a specific Jira issue |
| | **Get specific field info** | Retrieve information from a specific field of a Jira issue |
| | **Get field with image descriptions** | Retrieve field information with enhanced image description support |
| | **Get comments with image descriptions** | Retrieve comments with enhanced image description support |
| | **Get attachments content** | Retrieve content of attachments from a Jira issue |
| | **Get remote links** | Retrieve remote links associated with a Jira issue |
| | **Search index** | Search through indexed Jira data for enhanced discovery |
| | **Stepback search index** | Perform stepback search through indexed data |
| | **Stepback summary index** | Generate stepback summaries for indexed data |
| | **List indexes** | Retrieve available data indexes |
| **Create & update** | | |
| | **Create issue** | Create new Jira issues within a specified project |
| | **Update issue** | Modify fields of existing Jira issues |
| | **Set issue status** | Change the status of a specific Jira issue |
| | **Modify labels** | Add or remove labels from a specific Jira issue |
| | **Add comments** | Add new comments to a Jira issue |
| | **Update comment with file** | Update existing comments with file attachments |
| | **Add file to issue description** | Attach a file and embed it in the issue description |
| | **Link issues** | Create links between Jira issues |
| | **Index data** | Index Jira data for enhanced search and analytics |
| **Delete** | | |
| | **Remove index** | Remove existing data indexes |
| **Execute** | | |
| | **Execute generic rq** | Send custom HTTP requests to Jira API |

<Tip title="Vector Search Tools">
The tools **Index data**, **List indexes**, **Remove index**, **Search index**, **Stepback search index**, and **Stepback summary index** require PgVector configuration and an embedding model. These enable advanced semantic search capabilities across your Jira projects.
</Tip>
...
3. **Add Jira Toolkit to Conversation:**
   - Click the **`+`** icon in the chat input toolbar (tooltip: *Add files, agents, toolkits and more…*)
   - Hover over **Toolkits** in the popup menu
   - Search for your Jira toolkit by name
   - Enable the toolkit to add it to your conversation
4. **Verify Toolkit Added:** The Jira toolkit appears in the **PARTICIPANTS** panel on the right side under the **Toolkits** section.
5. **Use Toolkit in Chat:** You can now directly interact with your Jira projects and issues by asking questions or requesting actions that will trigger the Jira toolkit tools.
...
<video src="../../img/integrations/toolkits/jira/jira-chat-add.mp4" controls></video>
...
<Accordion title="Search & Discovery - Search using JQL">
**Chat Example:**
```
...
<Accordion title="Issue Management - Create Issue">
**Chat Example:**
```
...
<Accordion title="Communication - Add Comments">
**Chat Example:**
```
...
<Accordion title="Relationship Management - Link Issues">
**Chat Example:**
```
...
<Accordion title="Limited Number of Issues Returned">
...
<Accordion title="Atlassian API Connection Issues (Too Many Redirects)">
**Error Message:**
```
...
If you encounter issues not covered here or need additional assistance with Jira integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.

---
##  <Icon icon="circle-question" size={24} /> FAQ

<Accordion title="Can I use my regular Jira password for the ELITEA integration?">
...
<Accordion title="What permissions are absolutely necessary for the Jira API token to work with ELITEA?">
...
<Accordion title="What is the correct format for the Jira URL in the ELITEA credential configuration?">
...
<Accordion title="Can I use the same Jira credential across multiple toolkits and agents?">
...
<Accordion title="Why do I get 'Permission Denied' when using EPAM Jira (https://jiraeu.epam.com), even though my token worked before?">
...
<Accordion title="What are some best practices for using the Jira toolkit effectively?">
...
- Craft clear and unambiguous instructions within your ELITEA Agents to guide them in using the Jira toolkit effectively. Use the prompt examples provided in this documentation as a starting point.
...
<Accordion title="What are common use cases for Jira toolkit integration?">
...
<Note title="Documentation and Guides">
- **[How to Use Chat Functionality](../../how-tos/chat-conversations/how-to-use-chat-functionality)** - *Complete guide to using ELITEA Chat with toolkits for interactive GitHub operations.*
...
- **[Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas)** - *Discover how to create and configure GitHub toolkits directly from chat interface for streamlined workflow setup.*
- **[Create and Edit Pipelines from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-pipelines-from-canvas)** - *Guide to building and modifying pipelines from chat canvas for automated GitHub workflows.*
...
- **[How to Index Jira Data](../../how-tos/indexing/index-jira-data)** - *Complete guide for indexing your Jira projects and issues to enable advanced search and AI-powered analysis capabilities.*
...
<Note title="External Jira Resources">
