...
1. **Go to Toolkits:** Open the sidebar and select **[Toolkits & Indexes](../../menus/toolkits)**.
2. **Create new toolkit:** Select **`+ Toolkit`**.
3. **Select qTest:** Choose **qTest** from the list of available toolkit types.
    <img src="../../img/integrations/toolkits/qtest/qtest-toolkit-access.png" alt="qTest Toolkit Access" />
4. **Complete the fields:**
...
|:------------:|---------------|-----------------|
| **Read** | | |
| | **Find defects by test run ID** | Retrieve all defects associated with a specific test run |
| | **Find entity by ID** | Retrieve any qTest entity by its unique identifier |
| | **Find requirements by test case ID** | Retrieve all requirements linked to a specific test case |
| | **Find test case by ID** | Retrieve detailed information about a specific test case |
| | **Find test cases by requirement ID** | Retrieve all test cases linked to a specific requirement |
| | **Find test runs by test case ID** | Retrieve all test run instances for a specific test case |
| | **Get all test cases fields for project** | Retrieve all available custom and standard fields for test cases |
| | **Get modules** | Retrieve module IDs and names from the qTest project |
| | **Get test case versions** | Retrieve version history of a specific test case |
| | **List indexes** | List all available vector database collections |
| | **Search by dql** | Search for test cases using qTest Data Query Language (DQL) queries |
| | **Search entities by dql** | Search for various qTest entities using DQL queries |
| | **Search index** | Perform semantic search across indexed test case data |
| | **Stepback search index** | Perform stepback semantic search with query refinement |
| | **Stepback summary index** | Generate summaries from stepback search results |
| **Create & update** | | |
| | **Add file to test case** | Upload a file from artifact storage to a qTest test case or specific test step |
| | **Create test cases** | Create new test cases in qTest within a specified project and folder |
| | **Index data** | Index test case data into a vector database for semantic search |
| | **Link tests to Jira requirement** | Link test cases to Jira requirements |
| | **Link tests to qTest requirement** | Link test cases to qTest requirements |
| | **Update test case** | Modify fields of existing test cases |
| | **Update test run status** | Update the execution status of a manual test run and optionally add an execution note |
| | **Upload attachment to test run** | Upload a file to the test run context by attaching it to the underlying test case or to the latest test log |
| **Delete** | | |
| | **Delete test case** | Delete a test case from qTest |
| | **Remove index** | Delete indexed data from the vector database |
| **Execute** | | |
...
The tools **Index data**, **List indexes**, **Remove index**, **Search index**, **Stepback search index**, and **Stepback summary index** require PgVector configuration and an embedding model. These turn on advanced semantic search capabilities across your qTest projects.
</Tip>
...
3. **Add qTest toolkit:**
   - In the **"TOOLS"** section of the agent configuration, select the **"+Toolkit"** icon.
   ...
   - The toolkit will be added to your agent with the previously configured tools turned on.
    ...
    ![Add qTest toolkit to an agent](../../img/integrations/toolkits/qtest/qtest-agent-add.png)
...
3. **Add qTest toolkit:**
   - In the **"TOOLS"** section of the pipeline configuration, select the **"+Toolkit"** icon.
   - Select your configured qTest toolkit from the dropdown list.
   - The toolkit will be added to your pipeline with the previously configured tools turned on.

    ![Add qTest toolkit to a pipeline](../../img/integrations/toolkits/qtest/qtest-pipeline-add.png)
...
3. **Add qTest Toolkit to Conversation:**
   - Click the **`+`** icon in the chat input toolbar (tooltip: *Add files, agents, toolkits and more…*)
   - Hover over **Toolkits** in the popup menu.
   - Search for your qTest toolkit by name.
   - Enable the toolkit to add it to your conversation.
4. **Verify Toolkit Added:** The qTest toolkit appears in the **PARTICIPANTS** panel on the right side under the **Toolkits** section.
5. **Use Toolkit in Chat:** You can now directly interact with your qTest projects and test cases by asking questions or requesting actions that will trigger the qTest toolkit tools.

    ![Add qTest toolkit to a chat conversation](../../img/integrations/toolkits/qtest/qtest-chat-add.png)

<Tip>
You can also add the toolkit by typing **`#`** in the chat input box and searching for your qTest toolkit.
</Tip>
