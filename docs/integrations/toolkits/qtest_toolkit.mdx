...
<Note title="Search by DQL: lighter defaults and timeout protection">
The **Search by DQL** tool now uses lighter default request behavior for more predictable performance under heavier workloads.

By default:

- `append_test_steps` is `false`, so full test-step payloads are not fetched unless you explicitly request them
...
If your workflow needs the fuller payload, opt in explicitly by enabling those parameters for the specific call.
</Note>

<Info title="qTest API timeout protection">
qTest API requests now run with timeout protection. Slow or unresponsive qTest endpoints fail in a controlled way instead of waiting indefinitely. The default request timeout is 180 seconds and can be adjusted by platform configuration when needed.
</Info>
...
<Accordion title="Reporting on Test Case Coverage and Status using DQL Queries">
...
<Accordion title="Context and DQL Search Limit Issues">
...
<Accordion title="DQL Query Syntax and Module Issues">
...
If you encounter issues not covered in this guide or need additional assistance with qTest integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.
...
<Accordion title="My DQL queries return no results, but I know test cases exist. What's wrong?">
...
- **[How to Use Chat Functionality](../../how-tos/chat-conversations/how-to-use-chat-functionality)** - *Complete guide to using ELITEA Chat with toolkits for interactive qTest operations.*
...
- **[Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas)** - *Discover how to create and configure qTest toolkits directly from chat interface for streamlined workflow setup.*
- **[Create and Edit Pipelines from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-pipelines-from-canvas)** - *Guide to building and modifying pipelines from chat canvas for automated qTest workflows.*
...
- **[Credentials Documentation](../../menus/credentials)** - *Comprehensive guide to creating and managing credentials in ELITEA.*
