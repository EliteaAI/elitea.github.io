---
title: "Artifact"
description: "Connect ELITEA to the Artifact store — create, read, update, and delete files and documents within ELITEA's built-in storage."
---
...
<CardGroup cols={2}>
  <Card title="Store Temporary Data" icon="database">
    Save intermediate results, generated content, logs, and data retrieved during agent operations.
  </Card>
  <Card title="Manage Context" icon="brain">
    Preserve and retrieve context across multiple agent interactions or workflow steps.
  </Card>
  <Card title="Share Data Between Agents" icon="share-nodes">
    Enable collaboration by allowing different agents to access and modify files in shared buckets.
  </Card>
  <Card title="Process Multiple File Formats" icon="file-lines">
    Read from various document types (PDF, Excel, Word, images) and create text-based files.
  </Card>
  <Card title="Handle Large Outputs" icon="expand">
    Store data that exceeds LLM context windows or requires persistent access.
  </Card>
</CardGroup>

<Info title="Important: Temporary Storage">
...
|---------------|-------------------|----------------|--------------|
| **ListFiles** | Retrieves a list of files and folders in the bucket with download links | `bucket_name` (optional)<br />`folder` (optional)<br />`recursive` (optional)<br />`include` (optional, glob patterns)<br />`skip` (optional, glob patterns) | Check available files before processing; filter by extension or folder |
| **CreateFile** | Creates a new file or overwrites an existing file in the bucket | `filename` (required)<br />`filedata` (text content — provide either this or `filepath`)<br />`filepath` (optional, `/{bucket}/{filename}` — copies an existing file preserving binary format)<br />`bucket_name` (optional) | Store generated content, reports, or copy existing binary files |
| **ReadFile** | Reads and retrieves file content from the bucket | `filename` (required, or use `filepath`)<br />`filepath` (optional, `/{bucket}/{filename}` format)<br />`bucket_name` (optional)<br />`start_line` (optional)<br />`end_line` (optional)<br />`is_capture_image` (optional)<br />`page_number` (optional)<br />`sheet_name` (optional)<br />`extra_params` (optional, JSON string of per-file-type options) | Access previously stored data; partial reads via line range; per-type options via `extra_params` |
| **DeleteFile** | Permanently deletes a file from the bucket | `filename` (required)<br />`bucket_name` (optional) | Remove temporary or obsolete files |
| **AppendData** | Adds content to the end of an existing file | `filename` (required)<br />`filedata` (required)<br />`bucket_name` (optional)<br />`create_if_missing` (optional, default: `true`) | Build log files, accumulate test cases, expand datasets |
| **CreateNewBucket** | Creates a new storage bucket with custom retention | `bucket_name` (required)<br />`expiration_measure` (optional)<br />`expiration_value` (optional) | Organize different file groups, set custom retention periods |
| **Edit file** | Modifies specific content within an existing file using OLD/NEW markers | `file_path` (required)<br />`file_query` (required)<br />`branch` (optional)<br />`commit_message` (optional) | Make targeted text changes to supported file types |
| **Get file metadata** | Inspects a file and returns type, size, structure details, and available `read_file` options for that file type | `filepath` (optional, `/{bucket}/{filename}` format)<br />`filename` (optional)<br />`bucket_name` (optional) | Discover file structure (e.g. Excel sheet names, row counts) before reading; find supported `extra_params` |
| **Grep file** | Searches for a pattern within the content of a file | `file_path` (required)<br />`pattern` (required)<br />`bucket_name` (optional)<br />`is_regex` (optional)<br />`context_lines` (optional) | Find specific text or patterns inside a file |
...
| **Remove index** | Removes indexed data from the vector database | `index_name` (required) | Clean up obsolete indexes |
...
<Tip title="Get file metadata">
Use **Get file metadata** before reading complex files (Excel, PDF, images). It returns sheet names, row counts, embedded image counts, and the exact `extra_params` keys that **ReadFile** accepts for that specific file type.
</Tip>
...
For more details on canvas-based toolkit creation, see [How to Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas).
</Info>
...
<img src="../../img/how-tos/chat-conversations/attachments/enable-attachment-chat.gif" alt="Attachment Settings chat" />

<Info title="Attachment Integration">
...
<Info title="Reference">
For comprehensive details on attachment functionality, see [Attach Images and Files in Chat](../../how-tos/chat-conversations/attach-files).
</Info>
...
|--------------|---------------------|-----------------|
| **Text and Document Files** | `.txt`, `.md`, `.docx`, `.doc`, `.pdf` | Plain text, Markdown, Microsoft Word Documents (modern `.docx` and legacy `.doc`), Portable Document Format |
| **Data and Spreadsheet Files** | `.csv`, `.xlsx`, `.xls`, `.json`, `.jsonl`, `.xml`, `.yaml`, `.yml` | Comma Separated Values, Microsoft Excel (modern `.xlsx` and legacy `.xls`), JSON, JSON Lines, XML, YAML |
| **Presentation Files** | `.pptx`, `.ppt` | Microsoft PowerPoint Presentations (modern `.pptx` and legacy `.ppt`) |
| **Web and Markup Files** | `.html`, `.htm` | HTML web pages and markup documents |
| **Email Files** | `.eml`, `.msg` | Email files — RFC 822 (`.eml`) and Microsoft Outlook (`.msg`) formats, including attachments |
| **Programming and Code Files** | `.py`, `.js`, `.ts`, `.jsx`, `.tsx`, `.mjs`, `.cjs`, `.java`, `.cpp`, `.c`, `.h`, `.hpp`, `.cs`, `.go`, `.rb`, `.php`, `.swift`, `.kt`, `.rs`, `.scala`, `.sql`, `.sh`, `.bash`, `.zsh`, `.bat`, `.groovy`, `.lua`, `.r`, `.pl`, `.dart`, `.hs`, `.toml`, `.ini`, `.cfg`, `.conf`, `.env` | Python, JavaScript, TypeScript, React (JSX/TSX), Java, C/C++, C#, Go, Ruby, PHP, Swift, Kotlin, Rust, Scala, SQL, Shell/Batch scripts, Groovy, and many more |
| **Image Files** | `.jpg`, `.jpeg`, `.png`, `.gif`, `.bmp`, `.webp`, `.svg` | JPEG, PNG, GIF, Bitmap, WebP, and SVG Images — requires a multimodal LLM |

<Info title="Image Processing Requirements">
...
Pass text content via `filedata` — the file is stored with that content. **Any file extension is accepted.** Special format conversion applies only to two types:

| **Type** | **Extensions** | **How to provide `filedata`** |
|----------|----------------|-------------------------------|
| **Any text-based file** | Any extension — `.txt`, `.md`, `.json`, `.csv`, `.yaml`, `.yml`, `.xml`, `.html`, `.py`, `.js`, `.ts`, `.java`, `.sql`, `.sh`, `.log`, `.conf`, `.ini`, and any other text-representable format | Provide the file content as a plain string. Stored as-is. |
| **Excel Spreadsheet** | `.xlsx` | Provide `filedata` as a JSON string with sheet names as keys and 2D arrays as values. Automatically converted to Excel binary. |
| **Word Document** | `.docx` | Provide `filedata` as HTML or Markdown. Automatically converted to a Word document with tables, headings, and formatting preserved. |

<Info>
Binary formats not listed above (`.pdf`, `.pptx`, `.jpg`, `.png`, etc.) **cannot be generated from text content** via `filedata`. Passing text to these extensions will store the raw string, not a valid binary file.
</Info>

### File Types Supported for Editing (`Edit file` tool)

The `Edit file` tool uses targeted OLD/NEW marker edits and is restricted to text-editable extensions:

`.md`, `.txt`, `.csv`, `.json`, `.xml`, `.html`, `.yaml`, `.yml`, `.ini`, `.conf`, `.log`, `.sh`, `.py`, `.js`, `.ts`, `.jsx`, `.tsx`, `.java`, `.go`, `.rb`, `.php`, `.c`, `.cpp`, `.h`, `.hpp`, `.cs`, `.sql`, `.r`, `.m`, `.swift`, `.kt`, `.rs`, `.scala`
...
<Warning title="Data Sensitivity">
Artifact storage is designed for temporary operational data. Avoid storing:

- Passwords or API keys (use [Secrets](../../menus/settings/secrets) instead)
...
*   **Retention Management:** View and modify retention settings through the **[Artifacts page](../../menus/artifacts)**:
...
<Tip title="Retention Best Practices">
...
<Warning title="Storage Management">
...
<img src="../../img/integrations/toolkits/artifact/artifact-storage.png" alt="artifact-storage" width="150" />
</Warning>
...
<Accordion title="1. Storing Large Generated Outputs">
...
<Accordion title="2. Managing Workflow Context and State">
...
<Accordion title="3. Multi-Agent Collaboration">
...
<Tip title="Shared Bucket Best Practices">
...
<Accordion title="4. Extended Clipboard Functionality">
...
<Accordion title="5. Automated Logging and Audit Trails">
...
<Accordion title="6. Processing Pre-existing Data Files">
...
<Accordion title="7. Document Analysis and Extraction">
...
<Tip title="Multimodal Document Processing">
...
<Accordion title="8. Configuration Management">
...
<Accordion title="9. UI Test Execution and Results Storage">
...
<Tip title="Best Practices from Examples">
...
- **Manual Deletion:** Verify the file wasn't accidentally deleted by another agent or user. Check the [Artifacts page](../../menus/artifacts) for file history
</Accordion>

<Accordion title="Bucket Access Issues">
...
<Accordion title="Data Not Saving or Appending Correctly">
...
- **File Type Limitations:** For binary formats (PDF, images, etc.) use the `filepath` copy mode instead of `filedata`. For `.xlsx` provide JSON; for `.docx` provide HTML or Markdown.
...
<Accordion title="Toolkit Not Appearing in Dropdown">
...
<Accordion title="File Upload/Attachment Issues">
...
- **Review Artifacts Page:** Check the [Artifacts page](../../menus/artifacts) to see if files were uploaded successfully
</Accordion>

### Support Contact

For issues, questions, or assistance with Artifact toolkit integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.

---

## Frequently Asked Questions

<Accordion title="Can I create binary files like Excel or images in Artifacts?">
...
- YAML (`.yaml`, `.yml`)
- Markdown (`.md`)
- Code files (`.py`, `.js`, `.ts`, `.html`, `.sql`, etc.)
- **Excel files (`.xlsx`)** — provide filedata as JSON
- **Word documents (`.docx`)** — provide filedata as HTML or Markdown

**You can read** many binary file formats including Excel, Word, PDF, and images (with multimodal LLM support).
</Accordion>

<Accordion title="What is the maximum file size for artifacts?">
...
<Accordion title="How long are files stored in Artifacts?">
...
- By modifying bucket settings in the [Artifacts page](../../menus/artifacts)
...
<Accordion title="Can I recover deleted files from Artifacts?">
...
<Accordion title="How do I access and download files stored in Artifacts?">
You can access and manage artifact files through the **[Artifacts page](../../menus/artifacts)** in ELITEA:
...
<Accordion title="Can multiple agents share the same bucket?">
...
<Accordion title="Do I need credentials to use the Artifact Toolkit?">
...
<Accordion title="Can I use artifacts across different projects?">
...
<Accordion title="What happens when I enable attachments in a conversation?">
...
<Accordion title="What does filedata look like for creating a .docx file?">
The `CreateFile` tool accepts either **Markdown** or **HTML** as `filedata` when creating a `.docx` file. The SDK auto-detects the format based on the presence of HTML tags.
...
- Final review scheduled for July.
```
...
<h2>Summary</h2>
<p>This report covers Q2 results.</p>
<h2>Results Table</h2>
<table>
  <tr><th>Metric</th><th>Value</th></tr>
  <tr><td>Revenue</td><td>$1.2M</td></tr>
  <tr><td>Users</td><td>4,500</td></tr>
  <tr><td>Growth</td><td>18%</td></tr>
</table>
<h2>Notes</h2>
<ul>
  <li>All figures are preliminary.</li>
  <li>Final review scheduled for July.</li>
</ul>
```

Tables, headings, bold/italic, and lists are all preserved in the resulting Word document.
</Accordion>

<Accordion title="Can agents read image files?">
...
- **[Artifacts Menu Documentation](../../menus/artifacts)** - Comprehensive guide to managing artifacts through the UI
- **[Agents Menu Documentation](../../menus/agents)** - Complete guide to creating and configuring agents
- **[Pipelines Menu Documentation](../../menus/pipelines)** - Guide to building and managing pipelines
- **[Chat Documentation](../../menus/chat)** - Using chat conversations with toolkits
- **[Secrets Management](../../menus/settings/secrets)** - Secure storage for sensitive data
- **[How to Create and Edit Toolkits from Canvas](../../how-tos/chat-conversations/how-to-create-and-edit-toolkits-from-canvas)** - Creating toolkits from conversations
- **[Attach Files in Chat](../../how-tos/chat-conversations/attach-files)** - Using attachments with artifacts
</Info>