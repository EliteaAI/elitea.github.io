---
title: "Confluence"
description: "Connect ELITEA to Confluence — create, read, and update pages and spaces to automate documentation workflows."
---
...
<img src="../../img/integrations/toolkits/confluence/confluence-api-token-creation.gif" alt="Confluence-API_Token_Creation" />
...
3. **Select Confluence:** Choose **Confluence** as the credential type.
     ...
     ![Confluence Credential Access](../../img/integrations/toolkits/confluence/confluence-credential-access.gif)
...
6. **Save Credential:** Click **Save** to create the credential. It will be added to the credentials dashboard and available to use in toolkit configurations.
     ...
     ![Confluence Credential](../../img/integrations/toolkits/confluence/confluence-credential-create.gif)
...
3. **Select Confluence:** Choose **Confluence** from the list of available toolkit types.
     ...
     ![Confluence Toolkit Access](../../img/integrations/toolkits/confluence/confluence-toolkit-access.png)
...
4. **Configure Toolkit Details:**
   ...
   | **API Version** | Optional | Auto | REST API version used for Confluence operations. **Auto** (recommended) automatically selects V2 for Cloud or V1 for Server based on the credential's Hosting setting. Manually override only if Auto does not resolve correctly. See the note below for details |
   | **Space Key** | Required | — | Space key of the Confluence space to access (e.g., `DOCS`, `TEAM`). Found in space settings or in the URL when viewing the space |
...
<video src="../../img/integrations/toolkits/confluence/confluence-toolkit-create.mp4" controls></video>

<Info>
**API Version Selection**

The **API Version** field determines which Confluence REST API endpoints are used:

- **Auto** (default) — Automatically selects the API version based on the **Hosting** setting in the linked credential:
  - **Cloud hosting** (base URL contains `.atlassian.net`) → **V2** (`/wiki/api/v2`)
  - **Server hosting** (on-premises or Data Center) → **V1** (`/rest/api`)

- **V2** — Confluence Cloud REST API v2. Uses cursor-based pagination and storage/atlas_doc_format body representation. **Cloud-only** — selecting V2 against a Server/Data Center instance will fail.

- **V1** — Legacy Confluence REST API v1. **Required for Confluence Server / Data Center**; also still supported on Cloud for backward compatibility.

⚠️ **Important:** Confluence does not provide a public API v3. This differs from other Atlassian products like Jira which use v2/v3.
</Info>

#### Available Tools:

The Confluence toolkit provides 25 tools for interacting with Confluence spaces and pages, organized by functional categories:

| **Category** | **Tool Name** | **Description** |
|:------------:|---------------|-----------------|
| **Read** | | |
| | **Get page attachments** | Retrieves attachments from a specific page |
| | **Get page id by title** | Retrieves the page ID using the page title |
| | **Get page tree** | Retrieves the hierarchical structure of pages within a space |
| | **Get page with image descriptions** | Retrieves page content with image descriptions |
| | **Get pages with label** | Retrieves pages within a space that have a specific label |
| | **List indexes** | Lists available indexed collections for vector search |
| | **List pages with label** | Lists titles of pages within a space that have a specific label |
...
| | **Search index** | Performs searches across indexed content |
| | **Search pages** | Searches for pages within a specified space based on keywords |
| | **Site search** | Performs a search across the entire Confluence site |
| | **Stepback search index** | Performs advanced contextual searches with broader scope across indexed content |
| | **Stepback summary index** | Creates comprehensive summaries of indexed content with enhanced context |
| **Create & update** | | |
| | **Add file to page** | Attaches a file to a specific Confluence page |
| | **Create page** | Creates a new page within a specified space |
| | **Create pages** | Creates multiple pages within a space in bulk |
| | **Index data** | Creates searchable vector indexes of Confluence content |
| | **Update labels** | Adds or removes labels from a specific page |
| | **Update page by id** | Updates content of a page using its unique ID |
| | **Update page by title** | Updates content of a page using its title |
| | **Update pages** | Updates content of multiple pages in bulk |
| **Delete** | | |
| | **Delete page** | Deletes a specific page identified by its ID |
| | **Remove index** | Removes previously created search indexes |
| **Execute** | | |
| | **Execute generic confluence** | Sends custom HTTP requests to Confluence API for operations not covered by specific tools |

<Note>
**Read page by id: content format override**
...
The tools **Index data**, **List indexes**, **Remove index**, **Search index**, **Stepback search index**, and **Stepback summary index** require PgVector configuration and an embedding model. These enable advanced semantic search capabilities across your Confluence spaces and pages.
</Tip>
...
<img src="../../img/integrations/toolkits/confluence/confluence-agent-add.gif" alt="Confluence Agent" />
...
3. **Add Confluence Toolkit:** 
 ...
 <img src="../../img/integrations/toolkits/confluence/confluence-pipeline-add.gif" alt="Confluence Pipeline" />
...
3. **Add Confluence Toolkit to Conversation:**
   - Click the **`+`** icon in the chat input toolbar (tooltip: *Add files, agents, toolkits and more…*)
   - Hover over **Toolkits** in the popup menu
   - Search for your Confluence toolkit by name
   - Enable the toolkit to add it to your conversation
4. **Verify Toolkit Added:** The Confluence toolkit appears in the **PARTICIPANTS** panel on the right side under the **Toolkits** section.
5. **Use Toolkit in Chat:** You can now directly interact with your Confluence spaces and pages by asking questions or requesting actions that will trigger the Confluence toolkit tools.

<Tip>
You can also add the toolkit by typing **`#`** in the chat input box and searching for your Confluence toolkit.
</Tip>
...
<video src="../../img/integrations/toolkits/confluence/confluence-chat-add.mp4" controls></video>
