---
title: "ELITEA API Integration"
description: "This document provides comprehensive instructions for integrating with ELITEA AI agents using the REST API. Whether you're building custom applications, automating"
---
...
1. Navigate to **Settings** → **[Personal Access Tokens](../../menus/settings/personal-tokens)** in ELITEA
...
```bash
curl -X GET "https://next.elitea.ai/api/v2/..." \
...
|-------------|----------|
| Production | `https://next.elitea.ai/api/v2/` |
| Development | `https://dev.elitea.ai/api/v2/` |
...
```
POST {{BASE_URL}}elitea_core/conversations/prompt_lib/{{PROJECT_ID}}
```
...
| `source` | string | Source identifier (use `"elitea"`) |
| `participants` | array | Initial participants (typically empty) |

**curl Example:**

```bash
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/conversations/prompt_lib/{{PROJECT_ID}}" \
...
```
POST {{BASE_URL}}elitea_core/participants/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}
```
...
```bash
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/participants/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
  -d '{
    "entity_name": "application",
...
```
POST {{BASE_URL}}elitea_core/messages/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_UUID}}
```
...
| `await_task_timeout` | integer | Positive value = wait up to N seconds for reply (synchronous); `0` or `-1` = return immediately with `task_id` (async) |
| `return_task_id` | boolean | When `true` with `await_task_timeout <= 0`, returns task ID immediately instead of waiting |
| `attachments_info` | array | (Optional) List of attachment references |

**curl Example:**

```bash
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/messages/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_UUID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
  -d '{
...
```
POST {{BASE_URL}}elitea_core/predict/prompt_lib/{{PROJECT_ID}}/{{AGENT_VERSION_ID}}
```
...
| `return_chat_history` | boolean | (Optional) `true` to include the full chat history in the response, `false` to omit it (default) |
...
```bash
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/predict/prompt_lib/{{PROJECT_ID}}/{{AGENT_VERSION_ID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
  -d '{
    "user_input": "PROJ-123",
...
<Info title="When to Use Each Mode">
...
<Tip title="Use Cases for Direct Prediction">
...
```
GET {{BASE_URL}}elitea_core/conversations/prompt_lib/{{PROJECT_ID}}/?query={{NAME}}&limit=1&source=elitea
```

**curl Example:**

```bash
curl -X GET "https://next.elitea.ai/api/v2/elitea_core/conversations/prompt_lib/{{PROJECT_ID}}/?query=Support-TICKET-123&limit=1&source=elitea" \
  -H "Authorization: Bearer {{YOUR_PAT}}"
```
...
```
GET {{BASE_URL}}elitea_core/conversation/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}
```

**curl Example:**

```bash
curl -X GET "https://next.elitea.ai/api/v2/elitea_core/conversation/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}"
```
...
```javascript
const response = await fetch(`${API_URL}elitea_core/conversation/prompt_lib/${PROJECT_ID}/${CONVERSATION_ID}`, {
...
response = requests.get(
    f"{API_URL}elitea_core/conversation/prompt_lib/{PROJECT_ID}/{CONVERSATION_ID}",
...
<Tip title="Conversation Reuse Pattern">
...
Before uploading files, you must register an **artifact toolkit** as a participant and then designate it as the conversation's attachment storage. This is a two-part setup.

**1a. Add the artifact toolkit as a participant:**
...
Save the returned `id` as `{{ATTACHMENT_PARTICIPANT_ID}}`.
...
```
PUT {{BASE_URL}}elitea_core/conversation/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}
```
...
```json
{
  "attachment_participant_id": {{ATTACHMENT_PARTICIPANT_ID}}
}
```
...
```bash
curl -X PUT "https://next.elitea.ai/api/v2/elitea_core/conversation/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
  -d '{"attachment_participant_id": 78901}'
```
...
```
POST {{BASE_URL}}elitea_core/attachments/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}
```

**curl Example (file upload):**

```bash
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/attachments/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
...
response = requests.post(
    f"{API_URL}elitea_core/attachments/prompt_lib/{PROJECT_ID}/{CONVERSATION_ID}",
    headers={"Authorization": f"Bearer {PAT}"},
    files={
...
<Tip title="Supported File Types">
...
```bash
curl -X GET "https://next.elitea.ai/api/v2/elitea_core/conversations/prompt_lib/{{PROJECT_ID}}/?query=JIRA-PROJ-123&limit=1&source=elitea" \
  -H "Authorization: Bearer {{YOUR_PAT}}"
```
...
<Info title="Context Window Considerations">
...
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/predict/prompt_lib/630/93" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
...
CONV_RESPONSE=$(curl -s -X POST "https://next.elitea.ai/api/v2/elitea_core/conversations/prompt_lib/630" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
  -d '{"name": "Support-Case-001", "is_private": false, "source": "elitea", "participants": []}')

CONV_ID=$(echo $CONV_RESPONSE | jq -r '.id')
CONV_UUID=$(echo $CONV_RESPONSE | jq -r '.uuid')

# 2. Add agent participant
PART_RESPONSE=$(curl -s -X POST "https://next.elitea.ai/api/v2/elitea_core/participants/prompt_lib/630/$CONV_ID" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
...
# 3. Send message
curl -X POST "https://next.elitea.ai/api/v2/elitea_core/messages/prompt_lib/630/$CONV_UUID" \
  -H "Authorization: Bearer {{YOUR_PAT}}" \
  -H "Content-Type: application/json" \
...
|-----------|--------|----------|
| Create Conversation | POST | `elitea_core/conversations/prompt_lib/{{ProjectID}}` |
| Add Participant | POST | `elitea_core/participants/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Send Message | POST | `elitea_core/messages/prompt_lib/{{ProjectID}}/{{ConversationUUID}}` |
| Direct Prediction | POST | `elitea_core/predict/prompt_lib/{{ProjectID}}/{{AgentVersionID}}` |
| Check Conversation | GET | `elitea_core/conversations/prompt_lib/{{ProjectID}}/?query={{name}}&limit=1&source=elitea` |
| Get Conversation | GET | `elitea_core/conversation/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Add Toolkit Participant | POST | `elitea_core/participants/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Set Attachment Storage | PUT | `elitea_core/conversation/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Upload Attachment | POST | `elitea_core/attachments/prompt_lib/{{ProjectID}}/{{ConversationID}}` |

---

<Info title="Related Resources">
- [Personal Access Token Setup](../../getting-started/create-personal-access-token)
- [JIRA Automation with ELITEA](../../how-tos/credentials-toolkits/jira-automation-with-elitea)
