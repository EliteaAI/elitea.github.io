---
title: "Integrating Power Automate with ELITEA AI Agents"
description: "This document provides comprehensive instructions for integrating Microsoft Power Automate with ELITEA AI agents. By following this document, you can create automated workflows that trigger ELITEA agents from various Microsoft 365 events and enable ELITEA agents to perform actions in Microsoft 365 services."
---
...
<Info title="Two Integration Patterns">
...
|---------|--------|----------|
| Create Conversation | POST | `{{API_URL}}elitea_core/conversations/prompt_lib/{{ProjectID}}` |
| Add Agent Participant | POST | `{{API_URL}}elitea_core/participants/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Send Message | POST | `{{API_URL}}elitea_core/messages/prompt_lib/{{ProjectID}}/{{ConversationUUID}}` |
| Check Existing Conversation | GET | `{{API_URL}}elitea_core/conversations/prompt_lib/{{ProjectID}}/?query={{name}}&limit=1&source=alita` |
| Get Conversation Details | GET | `{{API_URL}}elitea_core/conversation/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Update Conversation (set attachment storage) | PUT | `{{API_URL}}elitea_core/conversation/prompt_lib/{{ProjectID}}/{{ConversationID}}` |
| Upload Attachment | POST | `{{API_URL}}elitea_core/attachments/prompt_lib/{{ProjectID}}/{{ConversationID}}` |

### Base URLs

| Environment | URL |
|-------------|-----|
| Production (next) | `https://next.elitea.ai/api/v2/` |
...
1. Navigate to **[Agents](../../menus/agents)** in your ELITEA project
...
4. Note the following values:
    - **Agent ID** and **Agent Version ID:** Visible in the Information section of the agent details page

       ![Agent Information](../../img/integrations/third-party-integrations/langfuse/agent-info.png)
    - **Project ID:** Located in Settings → **[General](../../menus/settings/general)**
    ![Project ID](../../img/integrations/third-party-integrations/langfuse/project-id.png)

<Tip title="Agent Instructions for Automation">
...
1. Go to **Settings** → **[Personal Access Tokens](../../menus/settings/personal-tokens)**
...
| API_URL | String | `https://next.elitea.ai/api/v2/` |
| AGENT_ID | String | `62` |
...
<Info title="Message Structure with Mentions">
...
<Tip title="Optional: Include Replies">
...
```
HTTP GET:
@{variables('API_URL')}elitea_core/conversations/prompt_lib/@{variables('PROJECT_ID')}/?query=Teams: {{displayName}}&limit=1&source=alita
```
...
```json
HTTP POST: @{variables('API_URL')}elitea_core/attachments/prompt_lib/@{variables('PROJECT_ID')}/@{ConversationID}
Body: {
...
```json
HTTP POST: @{variables('API_URL')}elitea_core/messages/prompt_lib/@{variables('PROJECT_ID')}/@{ConversationUUID}
Body: {
...
<Info title="Premium Connector Required">
...
Before uploading, the conversation needs a storage toolkit participant, and the conversation must be updated to point at it:

**1. Add the storage toolkit as a participant:**

```
HTTP POST: @{variables('API_URL')}elitea_core/participants/prompt_lib/@{variables('PROJECT_ID')}/@{ConversationID}
Body: {
  "entity_name": "toolkit",
  "entity_meta": {
    "id": {{toolkit_id}}
...
**2. Set the conversation's attachment participant:**

```
HTTP PUT: @{variables('API_URL')}elitea_core/conversation/prompt_lib/@{variables('PROJECT_ID')}/@{ConversationID}
Body: {
  "attachment_participant_id": {{participant_id}}
}
```

<Info title="Toolkit ID">
The `toolkit_id` refers to a storage toolkit configured in your ELITEA project (e.g., Artifacts toolkit). `participant_id` is the ID returned when that toolkit was added as a participant in step 1.
</Info>
...
1. **Method:** POST
2. **URL:** `https://next.elitea.ai/api/v2/elitea_core/attachments/prompt_lib/{{PROJECT_ID}}/{{CONVERSATION_ID}}`
3. **Headers:**
...
├─ Initialize Variables:
│   ├─ ChatHistory (String)
│   ├─ API_URL: "https://next.elitea.ai/api/v2/"
│   ├─ AGENT_ID: "62"
│   ├─ AGENT_VERSION_ID: "93"
│   ├─ PROJECT_ID: 630
│   ├─ attachments (String)
...
<Info title="Chat Type Filtering">
...
<Tip title="Enable Detailed Logging">
...
<Tip title="Finding Your Bot's User ID">
