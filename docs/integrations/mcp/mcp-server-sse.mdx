---
title: "Connect to Elitea MCP Server via SSE"
sidebarTitle: "Elitea MCP Server (SSE)"
description: "Connect VS Code to Elitea's MCP server over Server-Sent Events (SSE) using your project endpoint and bearer token."
---
...
- **MCP Server Endpoint**  
  ...
  ```
  Find your endpoint in Elitea → [Settings → AI Configuration](../../menus/settings/ai-configuration).

- **Authentication Token**  
  A personal access token from Elitea → [Settings → Personal Tokens](../../menus/settings/personal-tokens). Copy and store it securely as it will only be shown once.
...
<Info title="MCP Client Compatibility">
...
2. In Copilot Chat, select **Agent mode**.  
3. Click the **Tools** icon. 
    ![Open Tools](../../img/integrations/mcp/server-sse/chat-open-tools.png)
4. Add the MCP server:  
    ...
    - Enter your Server URL (e.g., `https://next.elitea.ai/app/PROJECT_ID/mcp`).
        ![Server URL](../../img/integrations/mcp/server-sse/server-url.png)
...
5. Choose where to save the configuration:
    ...
    - **User Settings** (applies globally to all VS Code workspaces)
     ...
     <video src="../../img/integrations/mcp/server-sse/add-mcp-server.mp4" controls></video>
...
1. In the Elitea web interface, tag your agents or pipelines with `mcp`.
   ![Tag agent as mcp](../../img/integrations/mcp/server-sse/tag-agent-mcp.png)
2. Only tagged agents and pipelines will be synced and available as tools in VS Code.

<Info title="Important Notes">
- Only the "**base**" version of each agent or pipeline tagged with `mcp` will be pulled and used. If you have multiple versions of the same agent/pipeline, ensure the version you want to use is the "**base**".
...
2. Select the **Make tools available by MCP** checkbox to enable MCP access for the selected tools.
     ...
     ![Make tools available by MCP](../../img/integrations/mcp/mcp-tools/make-tools-available-existing.png)
...
<Info>
For detailed instructions on making toolkit tools available by MCP, including security considerations and testing procedures, see [Make Tools Available by MCP](./make-tools-available-by-mcp).
</Info>
...
4. **Choose Your Tools**  
    ...
    ![MCP tools list](../../img/integrations/mcp/mcp-tools/mcp-client-testing.png)
...
6. **Confirm and Execute the Tool**  
    ...
    - Click **Allow** to proceed with running the tool.
...
7. **Configure Auto-Confirm (Optional)**  
    ...
    - Options include: current session only, this workspace, or always auto-confirm.
        <img src="../../img/integrations/mcp/server-sse/auto-confirm.png" alt="alt text" width="300" />

8. **Review Output**  
    - After execution, review the tool's output in the chat.
    - Use the results to continue your workflow or refine your request.

     <video src="../../img/integrations/mcp/server-sse/run-tool.mp4" controls width="600"></video>
...
<Accordion title="Enhancing Jira User Stories with BA Assistant Agent">
...
<Accordion title="Analyzing Confluence Documentation with Content Analyzer">
...
<Accordion title="Running Test Cases with QTest Integration">
...
<Accordion title="No Agents or Pipelines Are Shown">
...
2. **Token Expiry:** Ensure your token has not expired. Check the token's expiration date in Elitea Settings under [Personal Tokens](../../menus/settings/personal-tokens).
...
<Accordion title="VS Code MCP Server Connection Errors">
...
<Accordion title="Tools Not Executing or Hanging">
...
<Accordion title="Multiple Servers Conflict">
...
If you encounter issues not covered in this guide or need additional assistance with MCP Server integration, please refer to **[Contact Support](../../support/contact-support)** for detailed information on how to reach the ELITEA Support Team.
...
<Accordion title="What is the difference between Workspace and User MCP server configuration?">
...
<Accordion title="Can I use the same Elitea token for both Workspace and User configurations?">
Yes, you can use the same Elitea personal token in both configurations. However, for security reasons, it's recommended to:
- Store the token in **[Secrets](../../menus/settings/secrets)** in Elitea
...
<Accordion title="How do I know which version of my agent/pipeline will be used via MCP?">
...
<Accordion title="Can I use MCP tools outside of Agent mode?">
...
<Accordion title="Why do I need to restart the MCP server after tagging new agents?">
...
<Accordion title="What's the difference between agents, pipelines, and toolkit tools in MCP?">
...
<Accordion title="How many MCP servers can I configure in VS Code?">
...
<Accordion title="Do I need GitHub Copilot subscription to use Elitea MCP Server?">
...
<Accordion title="Can I control which toolkit tools are exposed via MCP?">
...
For more details, see [Make Tools Available by MCP](./make-tools-available-by-mcp).
</Accordion>

<Accordion title="What should I do if my token expires?">
When your Elitea personal token expires:

1. Go to Elitea → [Settings → Personal Tokens](../../menus/settings/personal-tokens)
...
<Accordion title="Why does the Allow button dropdown offer different auto-confirm options?">
...
- [Creating and Using MCP Client (STDIO)](./create-and-use-client-stdio)
- [Creating and Using MCP Server (STDIO)](./create-and-use-server-stdio)
- [Creating and Using Remote MCP](./create-and-use-remote-mcp)
</Info>
