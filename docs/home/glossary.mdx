---
title: "Glossary"
description: "This glossary provides definitions for key terms, components, and features used across the ELITEA platform. Understanding these terms will help you navigate and"
---
...
**Agent HUB**
: A dedicated shared library menu in ELITEA that provides centralized access to community-published agents. Similar to a marketplace, Agent HUB serves as a discovery hub where users can browse, search, and instantly use pre-built AI agents created by the ELITEA community. All agents undergo moderation review before publication, and users can like, fork, and add agents to conversations with one click.

**Agent Studio**
: **Legacy / alternate term:** An older or alternate name used in some documentation for **Agent HUB**.

**Analytics**
: A project-scoped settings section that provides real-time visibility into platform usage, including AI model calls, tool executions, user activity, agent runs, and health metrics for the currently selected project.

**Application**
: A purpose-built, application-level integration in ELITEA that delivers a focused workflow or interface for a specific domain, such as documentation generation or codebase exploration. Applications are complete solutions rather than general-purpose toolkits.
...
: A built-in module that enables Pandas-style data analysis in Chat and agents without creating a separate toolkit.
...
**Help Center**
: A central hub for ELITEA learning and reference materials, including documentation, release notes, video walkthroughs, tutorials, and interactive tours. Its content is configured by the platform administrator.

**High-tier**
: A model tier label in AI Configuration used for more capable, more expensive models intended for complex workflows such as multi-step reasoning or heavy tool usage.

**HITL Node**
: A pipeline control-flow node that introduces a human-in-the-loop review or decision point, allowing a pipeline to pause, present a message to a user, and continue based on the selected outcome or route.
...
**Inventory**
: An ELITEA Application that helps teams inspect a code estate, map key components, and understand service relationships and ownership before planning changes.
...
**Low-tier**
: A model tier label in AI Configuration used for cheaper, faster models intended for routine tasks such as formatting, simple edits, or lightweight automation.
...
: An optional toolkit setting that exposes selected toolkit tools to external MCP clients, allowing those tools to be discovered and used through Model Context Protocol connections.
...
**Monitoring**
: **Legacy / broader term:** An older or broader term used for analytics and tracking capabilities in ELITEA, including usage patterns, performance metrics, token consumption, engagement rates, and system health across agents, pipelines, and the overall platform.
...
**Planner (Module)**
: A built-in module that helps you break work into steps (a plan) and track progress within Chat or an agent conversation. Also called: Internal Planner.
...
**Project Background**
: The Markdown document inside **Project Context** where you write shared domain information, terminology, workflows, constraints, and other background details that ELITEA should apply across AI interactions in the project.

**Project Context**
: A project-level background document and feature that automatically injects shared project information into AI interactions so responses remain consistent, accurate, and aware of the team's domain, terminology, and constraints.
...
: A built-in module that enables secure Python code execution (via Pyodide) in Chat and agents, useful for calculations, data processing, and prototyping.
...
**Reindex**
: The operation that refreshes an existing index using its original configuration so updated source content is pulled into vector storage again. Reindexing is used to keep indexed data current without creating a brand-new index.
...
**S3 Storage**
: A storage type shown in the Artifacts storage selector for artifact buckets backed by Amazon S3-compatible object storage.

**Skill**
: A reusable, Markdown-based instruction set that you attach to agents to give them focused, named capabilities that can be invoked on demand during a conversation.

**Speech Recognition (ASR)**
: AI models and configuration used to convert spoken audio into text for ELITEA voice input features in Chat, Agents, and Pipelines. ELITEA can use server-side ASR models or fall back to browser speech recognition when available.

**Summarization**
: A context-management feature that automatically condenses older conversation content into shorter summaries to preserve continuity while reducing token usage and staying within model limits.

**Support Assistant**
: An AI-powered, context-aware help widget embedded in ELITEA that lets users ask questions, attach files, and get guidance without leaving the page they are working on.

**Smart Tools Selection (Module)**
...
**Swarm Mode (Module)**
: A module that enables multi-agent collaboration by allowing a parent agent to delegate tasks to child agents while sharing the same conversation context.

**Schedule**
: A time-based execution configuration used to run automated operations at defined intervals, such as scheduled pipeline entry-point runs or periodic index reindexing based on cron expressions.
...
**Text to Speech (TTS)**
: AI models and playback features that synthesize spoken audio from text so ELITEA can read responses aloud in Chat, Agents, and Pipelines. When no server-side TTS model is configured, ELITEA can fall back to the browser speech synthesis API.

**Token**
: A unit of text used by AI models to measure input and output size. Tokens are used for context windows, max completion limits, usage tracking, and personal access authentication tokens depending on context.

**Trigger**
: The mechanism that starts a workflow or action in ELITEA. In pipelines, the entry point can be triggered by a chat message, a schedule, or a webhook depending on the configured trigger type.
...
**Version**
: A snapshot or iteration of an entity (agent, pipeline, skills) that tracks changes and modifications over time. ELITEA maintains version history for entities, allowing users to compare different versions, revert to previous states, and track the evolution of their AI assets. Each version preserves the configuration and settings at the time it was created.
...
**Webhook**
: A callback-based integration mechanism used by some ELITEA features and external services to send event-driven HTTP notifications or trigger automated workflows when specific actions occur.

**Wikis**
: An ELITEA Application that turns a code repository into navigable documentation with architecture summaries, source-linked explanations, and project Q&A support.
